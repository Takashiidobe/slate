...
struct DispatchCfg {
    entry: usize,
    nodes: Vec<CfgNode>,
    backedges: Vec<CfgEdge>,
    natural_loops: Vec<NaturalLoop>,
    unsupported: BTreeSet<CfgUnsupported>,
}
...
impl DispatchCfg {
    fn analyze(region: &Region) -> Self {
        ...
        for (i, block) in region.blocks.iter().enumerate() {
            ...
            for op in &block.ops {
                ...
                {
                    label_to_block.insert(label.to_string(), i);
                }
            }
        }
        ...
        for (i, block) in region.blocks.iter().enumerate() {
            ...
            if !block_diverges(block) && i + 1 < region.blocks.len() {
        ...
        let entry = 0;
        let dominators = dominators(entry, &nodes);
        let backedges = nodes
            .iter()
            ...
            .filter(|edge| {
                dominators
                    .get(edge.from)
                    .is_some_and(|doms| doms.contains(&edge.to))
            })
            .cloned()
            .collect::<Vec<_>>();
        ...
        for cycle in cyclic_sccs(&nodes) {
            let entry_targets = cycle_entry_targets(&cycle, &nodes);
            if entry_targets.len() > 1 {
                unsupported.insert(CfgUnsupported::IrreducibleCycle(cycle));
            }
        }
    ...
    fn simple_self_loop(&self) -> Option<StructuredGotoLoop> {
        if !self.unsupported.is_empty()
            || self.natural_loops.len() != 1
            || self.backedges.len() != 1
        {
            return None;
        }
        let natural_loop = &self.natural_loops[0];
        if natural_loop.header != natural_loop.latch
            || natural_loop.nodes != BTreeSet::from([natural_loop.header])
            || natural_loop.entries.len() != 1
            || self.backedges[0].kind != CfgEdgeKind::Goto
        {
            return None;
        }
        let entry = &natural_loop.entries[0];
        if entry.to != natural_loop.header {
            return None;
        }
        let label = self.nodes[natural_loop.header].labels.first()?.clone();
        Some(StructuredGotoLoop {
...
impl<'a> Lowerer<'a> {
    ...
    fn lower_func(&mut self, op: &Op) -> Option<Item> {
        ...
        if body.blocks.len() > 1 {
            ...
            if !f.try_lower_structured_goto(body) {
...
impl<'a, 'b> FunctionLowerer<'a, 'b> {
    ...
    fn try_lower_structured_goto(&mut self, body: &Region) -> bool {
        ...
        if shape.preheader != cfg.entry || shape.header != 1 {
            return false;
        }
        let header = &body.blocks[shape.header];
        let Some(guard_index) = structured_loop_guard_index(header, &shape.label) else {
            return false;
        };

        self.lower_block(&body.blocks[shape.preheader]);
        let loop_body = self.capture_body(|this| {
            for op in &header.ops[..guard_index] {
                this.lower_op(op);
            }
            this.lower_structured_loop_guard(&header.ops[guard_index], &shape.label);
        });
        self.push_stmt(Stmt::Loop {
            label: None,
            body: loop_body,
        });
        for op in &header.ops[guard_index + 1..] {
            self.lower_op(op);
        }
        true
    }
    ...
    fn lower_structured_loop_guard(&mut self, op: &Op, label: &str) {
        if op.name == "cir.if" {
            self.push_structured_loop_break(op);
            return;
        }
        let Some(region) = op.regions.first() else {
            return;
        ...
        self.push_structured_loop_break(&block.ops[guard_index]);
    }

    fn push_structured_loop_break(&mut self, op: &Op) {
        let Some(cond) = op.operands.first() else {
            return;
        };
        let cond = self.operand_expr(cond);
        self.push_stmt(Self::guard_break(cond, None));
    }
...
fn block_labels(block: &Block) -> Vec<String> {
    block
        ...
        .filter_map(|op| {
            (op.name == "cir.label")
                .then(|| attr_str(op, "label").map(str::to_string))
                .flatten()
        })
        .collect()
}

fn structured_loop_guard_index(block: &Block, label: &str) -> Option<usize> {
    let mut matches = block
        .ops
        .iter()
        .enumerate()
        .filter_map(|(i, op)| op_is_loop_guard(op, label).then_some(i));
    let index = matches.next()?;
    matches.next().is_none().then_some(index)
}

fn op_is_loop_guard(op: &Op, label: &str) -> bool {
    if op.name == "cir.if" {
        return if_goto_to_label(op, label);
    }
    scoped_loop_guard_index(op, label).is_some()
}

fn scoped_loop_guard_index(op: &Op, label: &str) -> Option<usize> {
    if op.name != "cir.scope" {
        return None;
    }
    let region = op.regions.first()?;
    ...
    let index = ops
        .iter()
        .position(|inner| if_goto_to_label(inner, label))?;
    ops.iter()
        .skip(index + 1)
        .all(|inner| inner.name == "cir.yield")
        .then_some(index)
}

fn if_goto_to_label(op: &Op, label: &str) -> bool {
    if op.name != "cir.if" || op.regions.is_empty() {
        return false;
    }
    let then_matches = region_is_single_goto_to_label(&op.regions[0], label);
    let else_empty = op
        .regions
        .get(1)
        .is_none_or(|region| region.blocks.iter().all(|block| block.ops.is_empty()));
    then_matches && else_empty
}

fn region_is_single_goto_to_label(region: &Region, label: &str) -> bool {
    let mut ops = region
        .blocks
        .iter()
        .flat_map(|block| block.ops.iter())
        .filter(|op| op.name != "cir.yield");
...
fn collect_dispatch_successors(
    ...
    unsupported: &mut BTreeSet<CfgUnsupported>,
) {
    for op in ops {
        match op.name.as_str() {
            "cir.goto" => match attr_str(op, "label") {
                Some(label) => match label_to_block.get(label) {
                    Some(&to) => successors.push(CfgEdge {
                        from,
                        to,
                        kind: CfgEdgeKind::Goto,
                    }),
                    None => {
                        unsupported.insert(CfgUnsupported::UnknownLabel(label.to_string()));
                    }
                },
                None => {
                    unsupported.insert(CfgUnsupported::UnknownLabel(String::new()));
                }
            },
            "cir.br" => {
                if !op.operands.is_empty() && op.successors.is_empty() {
                    unsupported.insert(CfgUnsupported::DynamicBranch);
                }
                for target in &op.successors {
                    match block_to_state.get(target) {
                        Some(&to) => successors.push(CfgEdge {
                            from,
                            to,
                        ...
                        None => {
                            unsupported.insert(CfgUnsupported::UnknownBlock(target.clone()));
                        }
                    }
                }
            }
            "cir.indirect_br" => {
                unsupported.insert(CfgUnsupported::IndirectBranch);
            }
            _ => {}
        }
        for region in &op.regions {
...
fn dominators(entry: usize, nodes: &[CfgNode]) -> Vec<BTreeSet<usize>> {
...
fn natural_loop(backedge: &CfgEdge, nodes: &[CfgNode]) -> NaturalLoop {
    let predecessors = predecessors(nodes);
...
fn cyclic_sccs(nodes: &[CfgNode]) -> Vec<Vec<usize>> {
...
fn cycle_entry_targets(cycle: &[usize], nodes: &[CfgNode]) -> BTreeSet<usize> {
...
mod tests {
    ...
    fn label_op(label: &str) -> Op {
        let mut op = empty_op("cir.label");
        op.attrs.insert("label".into(), Attr::Str(label.into()));
        op
    }

    fn goto_op(label: &str) -> Op {
        let mut op = empty_op("cir.goto");
        op.attrs.insert("label".into(), Attr::Str(label.into()));
        op
    }

    fn br_op(targets: &[&str]) -> Op {
        let mut op = empty_op("cir.br");
        op.successors = targets.iter().map(|target| (*target).into()).collect();
        op
    }

    fn block(label: Option<&str>, ops: Vec<Op>) -> Block {
        Block {
    ...
    #[test]
    fn dispatch_cfg_records_forward_goto_without_loop() {
        let region = Region {
            blocks: vec![
                block(None, vec![goto_op("done")]),
                block(Some("bb1"), vec![label_op("done")]),
            ],
        };

        let cfg = DispatchCfg::analyze(&region);

        assert_eq!(cfg.entry, 0);
        assert_eq!(cfg.nodes[0].successors.len(), 1);
        assert_eq!(cfg.nodes[0].successors[0].to, 1);
        assert_eq!(cfg.nodes[1].labels, vec!["done"]);
        assert!(cfg.backedges.is_empty());
        assert!(cfg.natural_loops.is_empty());
        assert!(cfg.unsupported.is_empty());
    }

    #[test]
    fn dispatch_cfg_identifies_nested_backward_goto_as_natural_loop() {
        let mut if_op = empty_op("cir.if");
        if_op.regions.push(Region {
            blocks: vec![block(None, vec![goto_op("loop")])],
        });
        let region = Region {
            blocks: vec![
                block(None, Vec::new()),
                block(Some("bb1"), vec![label_op("loop"), if_op]),
            ],
        };

        let cfg = DispatchCfg::analyze(&region);

        assert_eq!(cfg.backedges.len(), 1);
        assert_eq!(cfg.backedges[0].from, 1);
        assert_eq!(cfg.backedges[0].to, 1);
        assert_eq!(cfg.natural_loops.len(), 1);
        let natural_loop = &cfg.natural_loops[0];
        assert_eq!(natural_loop.header, 1);
        assert_eq!(natural_loop.latch, 1);
        assert_eq!(natural_loop.nodes, BTreeSet::from([1]));
        assert_eq!(natural_loop.entries.len(), 1);
        assert_eq!(natural_loop.entries[0].from, 0);
        assert_eq!(natural_loop.entries[0].to, 1);
        assert!(natural_loop.exits.is_empty());
        assert!(cfg.unsupported.is_empty());
    }

    #[test]
    fn dispatch_cfg_rejects_dynamic_and_indirect_targets() {
        let mut dynamic_br = br_op(&[]);
        dynamic_br.operands.push("target".into());
        let region = Region {
            blocks: vec![
                block(None, vec![dynamic_br]),
                block(Some("bb1"), vec![empty_op("cir.indirect_br")]),
            ],
        };

        let cfg = DispatchCfg::analyze(&region);

        assert!(cfg.unsupported.contains(&CfgUnsupported::DynamicBranch));
        assert!(cfg.unsupported.contains(&CfgUnsupported::IndirectBranch));
    }

    #[test]
    fn dispatch_cfg_rejects_irreducible_multi_entry_cycle() {
        let region = Region {
            blocks: vec![
                block(None, vec![br_op(&["bb1", "bb2"])]),
                block(Some("bb1"), vec![br_op(&["bb2"])]),
                block(Some("bb2"), vec![br_op(&["bb1"])]),
            ],
        };
        ...
        assert!(
            cfg.unsupported
                .contains(&CfgUnsupported::IrreducibleCycle(vec![1, 2]))
        );
    }
