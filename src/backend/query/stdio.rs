...
use crate::backend::facts::file_ownership::{buf_ptr_var, match_gets_call, match_gets_loop};
use crate::backend::facts::{AstPath, FileOpenMode, FileUseKind, PathSegment};
use crate::backend::rust_ast::{
    BinOp, Block, Expr, Ident, IndentStmt, RustValue, Stmt, Type, UnaryOp,
};
use crate::function_identity::{Known, known_call};

use super::FileOwnership;

pub(in crate::backend) struct Plan {
...
fn gets_loop_stmt(
    handle: &str,
    gets: &crate::backend::facts::file_ownership::GetsLoopMatch,
) -> Stmt {
    ...
    let body = vec![
        IndentStmt {
            ...
            stmt: Stmt::Let {
                ...
                ty: Some(Type::Generic {
                    name: "Vec".into(),
                    args: vec![Type::Prim(crate::backend::rust_ast::Prim::U8)],
                }),
                init: Some(Expr::Call {
                    binding: crate::function_identity::CallBinding::Generated,
        ...
        IndentStmt {
            ...
            stmt: Stmt::If {
                cond: Expr::Binary {
                    op: crate::backend::rust_ast::BinOp::Eq,
