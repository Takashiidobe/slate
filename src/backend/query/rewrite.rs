...
use crate::backend::trace::{Pass, TraceFact, fact};

#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::backend) struct RuleIdentity {
    pub(in crate::backend) pass: Pass,
    pub(in crate::backend) name: String,
}

impl RuleIdentity {
    pub(in crate::backend) fn new(pass: Pass, name: impl Into<String>) -> Self {
        Self {
            pass,
            name: name.into(),
        }
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::backend) struct RuleCaseIdentity {
    pub(in crate::backend) rule: RuleIdentity,
    pub(in crate::backend) case: String,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::backend) struct CaseRejection {
    pub(in crate::backend) case: String,
    pub(in crate::backend) rejection: Rejection,
}
