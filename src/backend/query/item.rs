...
use crate::backend::rust_ast::{Expr, ExternDecl, IndentStmt, Item, Program};
use crate::backend::salsa::SalsaFacts;
use crate::backend::support::walk as mut_walk;
...
pub(in crate::backend) enum QueryDomain {
    Binding,
    Definition,
    EnumVariant,
    Expression,
...
pub(in crate::backend) enum QueryItem<'snapshot> {
...
pub(in crate::backend) enum Anchor {
    Binding {
        item_index: usize,
        path: AstPath,
        name: String,
...
pub(in crate::backend) struct StatementRef {
    pub(in crate::backend) item_index: usize,
    pub(in crate::backend) path: AstPath,
}

impl StatementRef {
    pub(in crate::backend) fn range(&self) -> StatementRange {
    ...
    pub(in crate::backend) fn container(&self) -> Option<super::StatementContainerRef> {
    ...
    pub(in crate::backend) fn index(&self) -> Option<usize> {
...
pub(in crate::backend) fn same_statement_container(declaration: &AstPath, other: &AstPath) -> bool {
...
#[derive(Clone)]
pub(in crate::backend) struct StatementMatch<const N: usize> {
    target: StatementRange,
}

impl<const N: usize> StatementMatch<N> {
    pub(in crate::backend) fn new(target: StatementRange) -> Self {
        Self { target }
    }

    pub(in crate::backend) fn target(&self) -> &StatementRange {
        &self.target
    }

    pub(in crate::backend) fn statement(&self, offset: usize) -> StatementRef {
        assert!(offset < N);
...
pub(in crate::backend) trait Matcher {
...
pub(in crate::backend) trait MatchCapture: Clone {
...
pub(in crate::backend) struct QueryRule<M: Matcher> {
...
impl<M: Matcher> QueryRule<M> {
    pub(in crate::backend) fn new(pass: Pass, rule: impl Into<String>, matcher: M) -> Self {
        Self {
    ...
    pub(in crate::backend) fn ordered_non_overlapping(mut self) -> Self {
        self.ordered_non_overlapping = true;
        self
    }
}

pub(in crate::backend) struct ItemCaseContext<'case, 'snapshot> {
    query: &'case QueryContext<'snapshot>,
    evidence: Vec<Evidence>,
}

impl<'snapshot> ItemCaseContext<'_, 'snapshot> {
    pub(in crate::backend) fn fact<T>(
        &mut self,
    ...
    pub(in crate::backend) fn attempt<T>(
        &mut self,
    ...
    pub(in crate::backend) fn expr(&self, site: &ExprSite) -> Option<&Expr> {
    ...
    pub(in crate::backend) fn call_args<const N: usize>(&self, call: &CallRecord) -> [ExprSite; N] {
        assert_eq!(N, call.args.len());
        std::array::from_fn(|index| call.args[index].clone())
    }

    pub(in crate::backend) fn statements<const N: usize>(
        &self,
    ...
    pub(in crate::backend) fn preceding_statements<const N: usize>(
        &self,
        matched: &StatementMatch<N>,
    ...
    pub(in crate::backend) fn lower_expr(
        &mut self,
    ...
    pub(in crate::backend) fn replace_function_body(
        &self,
    ...
    pub(in crate::backend) fn require(&self, condition: bool) -> Result<(), Rejection> {
        if condition {
            Ok(())
        } else {
            Err(self.reject())
    ...
    pub(in crate::backend) fn require_at(
        &self,
        condition: bool,
        predicate: Predicate,
        site: &ExprSite,
    ...
    pub(in crate::backend) fn reject(&self) -> Rejection {
        Rejection::new(
            Predicate::ItemGuard,
            None,
    ...
    pub(in crate::backend) fn reject_at(
        &self,
        predicate: Predicate,
        site: &ExprSite,
...
pub(in crate::backend) enum AnchoredEdit {
    Program {
    ...
    Function {
        ...
        replacement: crate::backend::rust_ast::FnDef,
    },
    Expression {
        target: ExprSite,
        replacement: Expr,
    ...
    Statement {
        target: StatementRange,
        replacement: Option<crate::backend::rust_ast::Stmt>,
    },
    Statements {
        target: StatementRange,
...
#[derive(Clone)]
pub(in crate::backend) enum DefinitionReplacement {
    #[expect(
        dead_code,
        reason = "query API surface not yet wired into a fixup rule"
    )]
...
pub(in crate::backend) struct EditSet {
    edits: Vec<AnchoredEdit>,
    evidence: Vec<Evidence>,
}

#[derive(Clone)]
pub(in crate::backend) struct FunctionEditTarget {
    item_index: usize,
    name: String,
}
...
impl EditSet {
    ...
    pub(in crate::backend) fn replace_expression(target: ExprSite, replacement: Expr) -> Self {
        Self {
    ...
    pub(in crate::backend) fn replace_program(
        target: super::ProgramRef,
        replacement: Program,
        removed: Vec<usize>,
    ) -> Self {
    ...
    pub(in crate::backend) fn push_replace_expression(
        &mut self,
        target: ExprSite,
        replacement: Expr,
    ) {
    ...
    pub(in crate::backend) fn push_replace_statement(
        &mut self,
        item_index: usize,
        path: AstPath,
        replacement: Option<crate::backend::rust_ast::Stmt>,
    ...
    pub(in crate::backend) fn extend(&mut self, other: EditSet) {
        self.edits.extend(other.edits);
        self.evidence.extend(other.evidence);
    }

    pub(in crate::backend) fn delete_definition(target: DefinitionSite) -> Self {
        Self::replace_definition(target, None)
    }

    pub(in crate::backend) fn replace_definition(
        target: DefinitionSite,
        replacement: Option<DefinitionReplacement>,
    ) -> Self {
        Self {
    ...
    pub(in crate::backend) fn replace_item(target: DefinitionSite, replacement: Item) -> Self {
    ...
    pub(in crate::backend) fn replace_extern_decl(
        target: DefinitionSite,
    ...
    pub(in crate::backend) fn replace_function(
        target: FunctionRef<'_>,
        replacement: crate::backend::rust_ast::FnDef,
    ) -> Self {
        Self {
    ...
    pub(in crate::backend) fn replace_statements(
        target: StatementRange,
        replacement: Vec<IndentStmt>,
    ) -> Self {
        Self {
    ...
    pub(in crate::backend) fn insert_items(
        index: usize,
        expected_len: usize,
        items: Vec<Item>,
    ) -> Self {
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord)]
pub(in crate::backend) struct EditSetSite(Vec<Anchor>);
...
pub(in crate::backend) struct ItemPlanBuilder {
...
impl ItemPlanBuilder {
    ...
    pub(in crate::backend) fn add_rule<M: Matcher>(
        &mut self,
        query: &QueryContext<'_>,
        rule: &QueryRule<M>,
    ) -> &mut Self {
    ...
    pub(in crate::backend) fn finish(self) -> ItemPlan {
        ItemPlan {
        ...
        for (item_index, item) in query.snapshot_program().items.iter().enumerate() {
            let count = match unwrap_cfg(item) {
                ...
                Item::Struct(definition) => match &definition.fields {
                    crate::backend::rust_ast::StructFields::Named(fields) => fields.len(),
                    crate::backend::rust_ast::StructFields::Tuple(fields) => fields.len(),
                },
                _ => 0,
            };
            fields.extend(
...
fn collect_match_arms<'snapshot>(
    ...
    for (index, indent) in body.iter().enumerate() {
        ...
        if let crate::backend::rust_ast::Stmt::Match { arms, .. } = &indent.stmt {
...
pub(in crate::backend) struct ItemPlan {
    plan: Plan<EditSet>,
}
...
pub(in crate::backend) struct ItemApplyReport {
    pub(in crate::backend) changed: bool,
    #[expect(
        dead_code,
        reason = "query API surface not yet wired into a fixup rule"
    )]
    pub(in crate::backend) planned: usize,
    #[cfg_attr(not(test), expect(dead_code, reason = "read only by tests"))]
    pub(in crate::backend) applied: usize,
    #[cfg_attr(not(test), expect(dead_code, reason = "read only by tests"))]
...
mod tests {
    use super::*;
    use crate::backend::rust_ast::{FnDef, RustValue, Stmt, Visibility};
    use crate::backend::salsa::SalsaFacts;
    use crate::backend::trace::NoopLogger;
