...
use crate::backend::facts::FunctionId;
use crate::backend::facts::atomic_locals::place_local_target;
...
pub(in crate::backend) struct ProgramReplacement {
    pub(in crate::backend) replacement: Program,
    pub(in crate::backend) removed: Vec<usize>,
}

pub(in crate::backend) fn rewrite_anonymous_structs(
...
pub(in crate::backend) fn rewrite_atomic_locals(
    query: &QueryContext<'_>,
...
pub(in crate::backend) fn rewrite_lazy_singletons(
    query: &QueryContext<'_>,
...
pub(in crate::backend) fn rewrite_printf_fallback(
    query: &QueryContext<'_>,
) -> QueryResult<ProgramReplacement> {
    let mut replacement = query.snapshot_program().clone();
...
pub(in crate::backend) fn rewrite_ptr_len(
    query: &QueryContext<'_>,
    set: PtrLenPlanSet,
) -> QueryResult<ProgramReplacement> {
...
pub(in crate::backend) fn rewrite_sort_search(
    query: &QueryContext<'_>,
) -> QueryResult<ProgramReplacement> {
...
fn rewrite_item(
    ...
    match item {
        ...
        Item::ExternBlock { decls, .. } => {
            ...
            for decl in decls {
                match decl {
                    crate::backend::rust_ast::ExternDecl::Fn(function) => {
                    ...
                    crate::backend::rust_ast::ExternDecl::Static { ty, .. } => {
