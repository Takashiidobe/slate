use crate::backend::facts::{GENERATED_C_STRING_READ_CALLEE, PrintfArgFact};
use crate::backend::rust_ast::{BinOp, Block, Expr, IndentStmt, Prim, RustValue, Stmt, Type};

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::backend) enum PrintfStream {
    Stdout,
    Stderr,
}
...
impl PrintfStream {
    ...
    pub(in crate::backend) fn plain_macro_name(self) -> &'static str {
        self.macro_names().1
    }
}

pub(in crate::backend) fn printf_macro(
    format: &[u8],
    args: &[Expr],
...
pub(in crate::backend) fn sprintf_format_expr(
    format: &[u8],
    args: &[Expr],
    arg_facts: &[PrintfArgFact],
) -> Option<Expr> {
...
pub(in crate::backend) fn sprintf_worst_case_len(
    format: &[u8],
    arg_facts: &[PrintfArgFact],
) -> Option<usize> {
    let parsed = parse_printf_format(format)?;
