...
use crate::backend::rust_ast::Type;
...
pub(in crate::backend) type QueryResult<T> = Result<Proof<T>, Rejection>;

#[derive(Debug, Clone, PartialEq, Eq, salsa::SalsaValue)]
pub(in crate::backend) struct Proof<T> {
    pub(in crate::backend) value: T,
...
impl<T> Proof<T> {
    ...
    pub(in crate::backend) fn into_parts(self) -> (T, Vec<Evidence>) {
        (self.value, self.evidence)
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::backend) struct Evidence {
    pub(in crate::backend) predicate: Predicate,
    pub(in crate::backend) site: ExprSite,
    pub(in crate::backend) detail: EvidenceDetail,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::backend) enum Predicate {
    Call,
    Binding,
    BindingUses,
    DefUse,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::backend) enum EvidenceDetail {
    IndexedCall {
        target: CallTarget,
        arity: usize,
    },
...
#[derive(Debug, Clone, PartialEq, Eq, salsa::SalsaValue)]
pub(in crate::backend) struct Rejection {
    pub(in crate::backend) predicate: Predicate,
    pub(in crate::backend) site: Option<ExprSite>,
    pub(in crate::backend) reason: RejectionReason,
    pub(in crate::backend) evidence: Vec<Evidence>,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::backend) enum RejectionReason {
    MissingEvidence,
    Contradicted,
    UnsupportedShape,
    Ambiguous,
