...
use crate::backend::facts::{CallsiteFact, walk};
...
pub(in crate::backend) fn collect_ascii_numeric_strings<'db>(
...
pub(in crate::backend) struct RewriteSnapshot<'db, 'a> {
    pub(in crate::backend) bindings: &'a [BindingFact<'db>],
    pub(in crate::backend) def_use: &'a [DefUseFact<'db>],
    pub(in crate::backend) values: &'a [ValueFact<'db>],
    pub(in crate::backend) string_buffers: &'a [StringBufferFact<'db>],
    pub(in crate::backend) string_pointer_views: &'a [StringPointerViewFact<'db>],
    pub(in crate::backend) string_libc_uses: &'a [StringLibcUseFact<'db>],
    pub(in crate::backend) printf_calls: &'a [PrintfCallFact<'db>],
    pub(in crate::backend) callsites: &'a [CallsiteFact<'db>],
    pub(in crate::backend) string_param_lifts: &'a [StringParamLiftFact<'db>],
}
...
pub(in crate::backend) fn collect_rewrite_facts_for_function<'db>(
    function: FunctionId<'db>,
    f: &FnDef,
...
pub(in crate::backend) fn use_allowed<'db>(
    function: FunctionId<'db>,
    use_path: &AstPath,
    facts: &RewriteSnapshot,
    binding: BindingId<'db>,
...
fn printf_allows_lift<'db>(
    function: FunctionId<'db>,
    printf: &crate::backend::facts::PrintfCallFact<'db>,
    facts: &RewriteSnapshot,
    binding: BindingId<'db>,
    liftable: &BTreeSet<BindingId<'db>>,
) -> bool {
...
fn printf_string_arg_allows_lift<'db>(
    ...
    binding: BindingId<'db>,
    liftable: &BTreeSet<BindingId<'db>>,
    arg: &crate::backend::facts::PrintfArgFact,
) -> bool {
...
pub(in crate::backend) struct Collected<'db> {
    pub(in crate::backend) buffers: Vec<StringBufferFact<'db>>,
    pub(in crate::backend) pointer_views: Vec<StringPointerViewFact<'db>>,
    pub(in crate::backend) libc_uses: Vec<StringLibcUseFact<'db>>,
}
...
impl<'db> BufferSummary<'db> {
    ...
    fn into_fact(self, function: FunctionId<'db>) -> StringBufferFact<'db> {
