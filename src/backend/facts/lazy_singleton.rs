...
use crate::backend::facts::{FunctionId, LazyInitSingletonFact, StaticDeclFact};
use crate::backend::idents::stmt_ident_count;
use crate::backend::rust_ast::{BinOp, Expr, IndentStmt, Prim, RustValue, Stmt, Type, UnaryOp};
...
pub(in crate::backend) fn match_body(body: &[IndentStmt]) -> Option<(String, String, Expr)> {
...
fn expr_references(expr: &Expr, name: &str) -> bool {
    crate::backend::idents::expr_ident_count(expr, name) > 0
}
    ...
    let static_count: usize = statics
        ...
        .map(|decl| {
            if decl.name == name {
                ...
                crate::backend::idents::expr_ident_count(&decl.init, name)
            }
        })
        .sum();
