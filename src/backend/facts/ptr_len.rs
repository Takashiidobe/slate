...
use crate::backend::facts::walk::{self, Bodies};
...
use crate::backend::rust_ast::{
    Block, Expr, FnDef, IndentStmt, Prim, RustValue, Stmt, Type, UnaryOp,
};
pub(in crate::backend) struct Snapshot<'db, 'a> {
    ...
    pub(in crate::backend) function_by_name: &'a BTreeMap<String, FunctionId<'db>>,
}
...
pub(in crate::backend) fn compute<'db>(
    ...
    let mut slices: Vec<PtrLenSliceFact<'db>> = candidates
        .iter()
...
pub(in crate::backend) struct Key<'db> {
    function: FunctionId<'db>,
    ptr: BindingId<'db>,
    len: BindingId<'db>,
}
...
fn loop_bound_index<'db>(
    ...
    if !matches!(op, crate::backend::rust_ast::BinOp::Lt)
