...
use crate::backend::rust_ast::{
    BinOp, Block, Expr, IndentStmt, Prim, RustValue, Stmt, Type, UnaryOp,
};
use crate::function_identity::{Known, known_call};
use std::collections::BTreeSet;

pub(super) type OwnedHeapUses<'db> = (
    Option<usize>,
    Option<BindingId<'db>>,
    Vec<BindingId<'db>>,
    Vec<HeapUseFact>,
...
pub(super) fn heap_uses_are_owned<'db>(
    function: FunctionId<'db>,
    body: &[IndentStmt],
    bindings: &[BindingFact<'db>],
    pointer_name: &str,
    candidate: &Candidate<'db>,
) -> Option<OwnedHeapUses<'db>> {
