...
use crate::backend::rust_ast::{BinOp, Expr, FnDef, Ident, IndentStmt, RustValue, Stmt, Type};
...
fn canonical_slice_loop_range<'db>(
    body: &[IndentStmt],
    index_name: &str,
    collector: &Collector<'db, '_>,
) -> Option<SliceLoopRange<'db>> {
...
fn negated_less_than(expr: &Expr) -> Option<(&Ident, &Expr)> {
    let Expr::Unary {
        op: crate::backend::rust_ast::UnaryOp::Not,
        expr,
    } = expr
    else {
        return None;
...
fn integer_value(expr: &Expr) -> Option<i128> {
    match expr {
        ...
        Expr::Unary {
            op: crate::backend::rust_ast::UnaryOp::Neg,
            expr,
