...
pub(in crate::backend) use crate::backend::support::walk::{
    nested_bodies_with_path, nested_body_vecs_with_path, with_path_segment,
};
use salsa::plumbing::{AsId, FromId};

pub(in crate::backend) fn body_exprs(body: &[IndentStmt], f: &mut impl FnMut(&Expr)) {
    for stmt in body {
        stmt_exprs(&stmt.stmt, f);
    }
}

pub(in crate::backend) fn block_exprs(block: &Block, f: &mut impl FnMut(&Expr)) {
...
pub(in crate::backend) fn stmt_exprs(stmt: &Stmt, f: &mut impl FnMut(&Expr)) {
    match stmt {
        Stmt::Let { init, .. } => {
...
pub(in crate::backend) fn exprs(expr: &Expr, f: &mut impl FnMut(&Expr)) {
    f(expr);
    match expr {
        Expr::Value(_)
        | Expr::Str(_)
...
pub(in crate::backend) fn body_exprs_with_path(
    body: &[IndentStmt],
...
pub(in crate::backend) fn block_exprs_with_path(
    block: &Block,
    path: &mut Vec<PathSegment>,
    f: &mut impl FnMut(&Expr, &mut Vec<PathSegment>),
) {
...
pub(in crate::backend) fn stmt_exprs_with_path(
    stmt: &Stmt,
    path: &mut Vec<PathSegment>,
    f: &mut impl FnMut(&Expr, &mut Vec<PathSegment>),
) {
...
pub(in crate::backend) fn exprs_with_path(
    expr: &Expr,
    path: &mut Vec<PathSegment>,
    f: &mut impl FnMut(&Expr, &mut Vec<PathSegment>),
) {
...
pub(in crate::backend) fn body_exprs_any(
    body: &[IndentStmt],
...
pub(in crate::backend) fn block_exprs_any(
    block: &Block,
    pred: &mut impl FnMut(&Expr) -> bool,
) -> bool {
...
pub(in crate::backend) fn stmt_exprs_any(
    stmt: &Stmt,
    pred: &mut impl FnMut(&Expr) -> bool,
) -> bool {
    match stmt {
...
pub(in crate::backend) fn exprs_any(expr: &Expr, pred: &mut impl FnMut(&Expr) -> bool) -> bool {
    if pred(expr) {
        return true;
    }
    match expr {
...
pub(in crate::backend) fn path_starts_with(path: &[PathSegment], prefix: &[PathSegment]) -> bool {
    path.len() >= prefix.len() && path[..prefix.len()] == *prefix
}

pub(in crate::backend) fn paths_overlap(a: &[PathSegment], b: &[PathSegment]) -> bool {
    path_starts_with(a, b) || path_starts_with(b, a)
}

pub(in crate::backend) fn target_expr_at_path<'a>(
    program: &'a Program,
    item_index: usize,
    path: &AstPath,
) -> Option<&'a Expr> {
...
pub(in crate::backend) type Bodies<'a, 'db> = BTreeMap<FunctionId<'db>, &'a FnDef>;
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub(in crate::backend) struct RawBinding {
    pub(in crate::backend) id: salsa::Id,
    function: salsa::Id,
    name: String,
    kind: BindingKind,
    path: AstPath,
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub(in crate::backend) struct RawBindingType {
    binding: salsa::Id,
    ty: Type,
    rendered: String,
}
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub(in crate::backend) struct RawLoop {
    id: salsa::Id,
    function: salsa::Id,
    kind: LoopKind,
    path: AstPath,
...
pub(in crate::backend) fn hydrate_binding<'db>(raw: &RawBinding) -> BindingFact<'db> {
    BindingFact {
...
pub(in crate::backend) fn hydrate_binding_type<'db>(raw: &RawBindingType) -> BindingTypeFact<'db> {
    BindingTypeFact {
...
pub(in crate::backend) fn hydrate_loop<'db>(raw: &RawLoop) -> LoopFact<'db> {
    LoopFact {
...
pub(in crate::backend) struct BaseWalk {
...
impl BaseWalk {
    pub(in crate::backend) fn new(
        db: &dyn crate::backend::salsa::FixupDb,
        program: &Program,
    ) -> Self {
        Collector::new(db).collect(program)
    }

    pub(in crate::backend) fn for_function(
        db: &dyn crate::backend::salsa::FixupDb,
        function: salsa::Id,
        f: &FnDef,
    ) -> Self {
    ...
    pub(in crate::backend) fn function_item_index(
        &self,
    ...
    pub(in crate::backend) fn function_facts<'db>(&self) -> Vec<FunctionFact<'db>> {
        self.functions
            .iter()
    ...
    pub(in crate::backend) fn binding_facts<'db>(&self) -> Vec<BindingFact<'db>> {
        self.bindings.iter().map(hydrate_binding).collect()
    }

    pub(in crate::backend) fn binding_type_facts<'db>(&self) -> Vec<BindingTypeFact<'db>> {
        self.binding_types
            .iter()
    ...
    pub(in crate::backend) fn loop_facts<'db>(&self) -> Vec<LoopFact<'db>> {
        self.loops.iter().map(hydrate_loop).collect()
    }
}

struct Collector<'a> {
    db: &'a dyn crate::backend::salsa::FixupDb,
    base: BaseWalk,
}

impl<'a> Collector<'a> {
    fn new(db: &'a dyn crate::backend::salsa::FixupDb) -> Self {
        Self {
            db,
...
pub(in crate::backend) fn expr_at_body_path<'db, 'a>(
    bodies: &Bodies<'db, 'a>,
...
pub(in crate::backend) fn expr_in_body<'a>(
    body: &'a [IndentStmt],
    path: &[PathSegment],
) -> Option<&'a Expr> {
    let [PathSegment::Stmt(index), rest @ ..] = path else {
