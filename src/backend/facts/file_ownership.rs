...
use crate::backend::rust_ast::{BinOp, Expr, IndentStmt, Path, RustValue, Stmt, Type, UnaryOp};
...
pub(in crate::backend) struct GetsLoopMatch {
    pub buf_name: String,
    pub buf_len: i64,
}
...
pub(in crate::backend) fn match_gets_loop(
...
pub(in crate::backend) fn match_gets_call(
    body: &[IndentStmt],
    use_index: usize,
) -> Option<GetsLoopMatch> {
...
pub(in crate::backend) fn buf_ptr_var(expr: &Expr) -> Option<String> {
