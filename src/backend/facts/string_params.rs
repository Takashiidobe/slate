...
use crate::backend::facts::{CallSignatureSource, CallsiteFact, walk};
use crate::backend::rust_ast::{Expr, FnDef, Prim, Type, Visibility};
...
pub(in crate::backend) struct Candidate<'db> {
    key: Key<'db>,
    function_name: String,
    index: usize,
}
pub(in crate::backend) struct Snapshot<'db, 'a> {
    pub(in crate::backend) bindings: &'a [BindingFact<'db>],
    pub(in crate::backend) binding_types: &'a [BindingTypeFact<'db>],
...
pub(in crate::backend) fn compute<'db>(
    bodies: &Bodies,
    facts: &Snapshot,
...
pub(in crate::backend) fn candidates_for_function<'db>(
    function: FunctionId<'db>,
    f: &FnDef,
...
fn binding_is_liftable_source<'db>(
    ...
    facts.string_buffer(binding).is_some_and(|buffer| {
        matches!(
            ...
            && buffer
                .candidates
                .contains(&crate::backend::facts::StringRecoveryCandidate::BorrowedStr))
