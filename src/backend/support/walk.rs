...
use crate::backend::rust_ast::{Block, Expr, IndentStmt, Stmt};

pub(in crate::backend) fn with_path_segment<R>(
...
pub(in crate::backend) fn nested_bodies_with_path(
    stmt: &Stmt,
    path: &mut Vec<PathSegment>,
...
/// Same traversal as `nested_bodies_with_path`, but with `stmt`'s lifetime
/// named explicitly so a caller collecting borrowed `&'a [IndentStmt]`
/// slices across the whole walk (rather than just visiting-and-dropping
/// each one) can do so - `impl FnMut(&[IndentStmt], ..)` alone would bind a
/// fresh, call-local lifetime unrelated to `stmt`'s.
pub(in crate::backend) fn nested_body_vecs_with_path<'a>(
    stmt: &'a Stmt,
    path: &mut Vec<PathSegment>,
    f: &mut impl FnMut(&'a [IndentStmt], &mut Vec<PathSegment>),
) {
    match stmt {
        Stmt::If {
            then_body,
...
pub(in crate::backend) fn nested_bodies_mut_with_path(
    stmt: &mut Stmt,
    path: &mut Vec<PathSegment>,
...
pub(in crate::backend) fn nested_body_vecs_mut_with_path(
    stmt: &mut Stmt,
    path: &mut Vec<PathSegment>,
...
pub(in crate::backend) fn stmt_expr_any(stmt: &Stmt, pred: &mut impl FnMut(&Expr) -> bool) -> bool {
    match stmt {
...
pub(in crate::backend) fn body_expr_any(
    body: &[IndentStmt],
...
pub(in crate::backend) fn expr_any(expr: &Expr, pred: &mut impl FnMut(&Expr) -> bool) -> bool {
    if pred(expr) {
        return true;
    }
    match expr {
...
pub(in crate::backend) fn body_exprs_mut_with(
    body: &mut [IndentStmt],
...
pub(in crate::backend) fn block_exprs_mut_with(
    block: &mut Block,
    f: &mut impl FnMut(&mut Expr) -> bool,
) {
...
pub(in crate::backend) fn stmt_exprs_mut_with(
    stmt: &mut Stmt,
    f: &mut impl FnMut(&mut Expr) -> bool,
) {
    match stmt {
...
pub(in crate::backend) fn exprs_mut_with(expr: &mut Expr, f: &mut impl FnMut(&mut Expr) -> bool) {
    if !f(expr) {
        return;
    }
    match expr {
...
pub(in crate::backend) fn body_exprs_mut_with_path(
    body: &mut [IndentStmt],
    path: &mut Vec<PathSegment>,
...
pub(in crate::backend) fn block_exprs_mut_with_path(
    block: &mut Block,
    path: &mut Vec<PathSegment>,
    f: &mut impl FnMut(&mut Expr, &mut Vec<PathSegment>) -> bool,
) {
...
pub(in crate::backend) fn stmt_exprs_mut_with_path(
    stmt: &mut Stmt,
    path: &mut Vec<PathSegment>,
    f: &mut impl FnMut(&mut Expr, &mut Vec<PathSegment>) -> bool,
) {
...
pub(in crate::backend) fn exprs_mut_with_path(
    expr: &mut Expr,
    path: &mut Vec<PathSegment>,
    f: &mut impl FnMut(&mut Expr, &mut Vec<PathSegment>) -> bool,
) {
