...
#define obstack_begin(obstack, size)                                           \
  _obstack_begin((obstack), (size), 0, (void *(*)(long))obstack_chunk_alloc,   \
                 (void (*)(void *))obstack_chunk_free)
#define obstack_specify_allocation(obstack, size, alignment, chunkfun,         \
                                   freefun)                                    \
  _obstack_begin((obstack), (size), (alignment), (void *(*)(long))(chunkfun),  \
                 (void (*)(void *))(freefun))
#define obstack_specify_allocation_with_arg(obstack, size, alignment,          \
...
#define obstack_1grow_fast(obstack, value) (*((obstack)->next_free)++ = (value))
...
#define obstack_empty_p(obstack)                                               \
  ((obstack)->chunk->prev == 0 &&                                              \
   (obstack)->next_free == __PTR_ALIGN((char *)(obstack)->chunk,               \
                                       (obstack)->chunk->contents,             \
                                       (obstack)->alignment_mask))

#define obstack_make_room(obstack, length)                                     \
  ((obstack)->temp.tempint = (length),                                         \
   ((obstack)->next_free + (obstack)->temp.tempint > (obstack)->chunk_limit    \
...
#define obstack_blank(obstack, length)                                         \
  ((obstack)->temp.tempint = (length),                                         \
   ((obstack)->chunk_limit - (obstack)->next_free < (obstack)->temp.tempint    \
        ? (_obstack_newchunk((obstack), (obstack)->temp.tempint), 0)           \
        : 0),                                                                  \
...
#define obstack_free(obstack, object)                                          \
  ((obstack)->temp.tempint = (char *)(object) - (char *)(obstack)->chunk,      \
   ((obstack)->temp.tempint > 0 &&                                             \
        ...
        : (__obstack_free)((obstack), (obstack)->temp.tempint +                \
                                          (char *)(obstack)->chunk)))
