...
pub fn get_function_modifier_tokens(bitfield: &Bitfield) -> Option<TokenStream> {
    (!bitfield.has_ignored_fields()
        && !bitfield.is_array_heap()
        && !bitfield.fields().iter().any(|field| {
            field.bits() > 128
                && field
                    .arguments()
                    .is_some_and(|arguments| arguments.from_function().is_some())
        })
...
pub fn generate_setting_field_to_zero_tokens(bitfield: &Bitfield, field: &Field) -> TokenStream {
    let value_tokens = match field.spanned_data_type_token().data_type() {
        ...
        DataType::Custom => {
            if !bitfield.is_integer_backed() && field.bits() > 128 {
                let len = field.bits().div_ceil(8) as usize;
                generate_custom_field_from_bits_tokens(field, quote! { [0u8; #len] })
            } else {
                generate_custom_field_from_bits_tokens(field, quote! { 0 })
            }
        },
        DataType::Array {
            length,
...
pub fn generate_setting_field_from_variable_tokens(
    ...
    let is_array_field = matches!(
        ...
        DataType::Array { .. }
    ) || (!bitfield.is_integer_backed()
        && field.bits() > 128
        && matches!(field.spanned_data_type_token().data_type(), DataType::Custom));
    ...
    let value_variable_tokens = get_value_variable_tokens(bitfield, field);
...
fn get_value_variable_tokens(bitfield: &Bitfield, field: &Field) -> TokenStream {
    match field.spanned_data_type_token().data_type() {
        DataType::Custom => {
            if !bitfield.is_integer_backed() && field.bits() > 128 {
                generate_custom_field_from_bits_tokens(field, quote! { value })
            } else {
...
fn generate_setting_field_without_setter_tokens(
    ...
    if !bitfield.is_integer_backed()
        ...
        && field.bits() > 128
    {
        let into_bits_tokens = generate_custom_field_into_bits_tokens(field, value_tokens);
        return quote! {
            {
                let __src = #into_bits_tokens;
                let mut __i: u32 = 0;
                while __i < #field_bits_tokens {
                    let bit_index = #field_offset_tokens + __i;
                    let byte_idx = (bit_index / 8) as usize;
