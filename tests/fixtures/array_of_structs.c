...
// REWRITES-NEXT: let {{_v[0-9]+}}: i64 = 0;
// REWRITES-NEXT: let {{_v[0-9]+}}: i64 = 1;
// REWRITES-NEXT: unsafe { printf({{_v[0-9]+}} as *const i8, ps[({{_v[0-9]+}} as usize)].x + ps[({{_v[0-9]+}} as usize)].y) };
// REWRITES-NEXT: *init = [Point { x: 10, y: 20 }, Point { x: 30, y: 40 }];
// REWRITES-NEXT: let {{_v[0-9]+}}: *mut i8 = b"%d\n\0".as_ptr() as *mut i8;
// REWRITES-NEXT: let {{_v[0-9]+}}: i64 = 0;
// REWRITES-NEXT: let {{_v[0-9]+}}: i64 = 1;
// REWRITES-NEXT: unsafe { printf({{_v[0-9]+}} as *const i8, init[({{_v[0-9]+}} as usize)].y + init[({{_v[0-9]+}} as usize)].x) };
