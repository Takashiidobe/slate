...
// LOWERING-NEXT:     let mut b: i32 = 0;
// LOWERING-NEXT:     let mut __retval: i32 = 0;
// LOWERING-NEXT:     useAdd = {{arg[0-9]+}};
// LOWERING-NEXT:     a = {{arg[0-9]+}};
// LOWERING-NEXT:     b = {{arg[0-9]+}};
// LOWERING-NEXT:     let {{_v[0-9]+}}: i32 = useAdd;
...
// REWRITES-NEXT: let mut useAdd: i32 = {{arg[0-9]+}};
// REWRITES-NEXT: let mut a: i32 = {{arg[0-9]+}};
// REWRITES-NEXT: let mut b: i32 = {{arg[0-9]+}};
// REWRITES-NEXT: let mut __retval: i32 = 0;
// REWRITES-NEXT: let {{_v[0-9]+}}: bool = useAdd != 0;
// REWRITES-NEXT: let {{_v[0-9]+}}: Option<unsafe extern "C" fn(i32, i32) -> i32> = if {{_v[0-9]+}} { Some(add) } else { Some(sub) };
// REWRITES-NEXT: __retval = unsafe { {{_v[0-9]+}}.unwrap()(a, b) };
// REWRITES-NEXT: return __retval;
// REWRITES-NEXT: }
// REWRITES-EMPTY:
...
// REWRITES-NEXT: let {{_v[0-9]+}}: bool = useAdd != 0;
// REWRITES-NEXT: op = if {{_v[0-9]+}} { Some(add) } else { Some(sub) };
// REWRITES-NEXT: let {{_v[0-9]+}}: *mut i8 = b"%d %d %d %d\n\0".as_ptr() as *mut i8;
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = 10;
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = 3;
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = unsafe { op.unwrap()({{_v[0-9]+}}, {{_v[0-9]+}}) };
