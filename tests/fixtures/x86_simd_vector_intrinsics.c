...
// REWRITES-NEXT: unsafe { ssse3_probe({{_v[0-9]+}}) };
// REWRITES-NEXT: let {{_v[0-9]+}}: *mut i32 = avx2_out.as_mut_ptr() as *mut i32;
// REWRITES-NEXT: unsafe { avx2_probe({{_v[0-9]+}}) };
...
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = unsafe { printf({{_v[0-9]+}} as *const i8, sse2_out[({{_v[0-9]+}} as usize)], sse2_out[({{_v[0-9]+}} as usize)], sse2_out[({{_v[0-9]+}} as usize)], sse2_out[({{_v[0-9]+}} as usize)]) };
// REWRITES-NEXT: let {{_v[0-9]+}}: *mut i8 = b"%d %d\n\0".as_ptr() as *mut i8;
// REWRITES-NEXT: let {{_v[0-9]+}}: i64 = 0;
// REWRITES-NEXT: let {{_v[0-9]+}}: i64 = 15;
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = unsafe { printf({{_v[0-9]+}} as *const i8, ssse3_out[({{_v[0-9]+}} as usize)] as i32, ssse3_out[({{_v[0-9]+}} as usize)] as i32) };
