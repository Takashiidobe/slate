...
// COMMON-LOWERING-NEXT:     let {{__v[0-9]+}}: *mut i32 = make({{__v[0-9]+}});
...
// COMMON-LOWERING-NEXT: }
// COMMON-LOWERING-EMPTY:
// COMMON-LOWERING-NEXT: fn make({{arg[0-9]+}}: i32) -> *mut i32 {
...
// COMMON-REWRITES-NEXT:     let {{__v[0-9]+}}: *mut i32 = Box::into_raw(make(4)).cast::<i32>();
...
// COMMON-REWRITES-NEXT: }
// COMMON-REWRITES-EMPTY:
// COMMON-REWRITES-NEXT: fn make({{arg[0-9]+}}: i32) -> Box<[i32]> {
// COMMON-REWRITES-NEXT:     let mut n: i32 = {{arg[0-9]+}};
// COMMON-REWRITES-NEXT:     let mut p: *mut i32 = std::ptr::null_mut();
// COMMON-REWRITES-NEXT:     let {{__v[0-9]+}}: u64 = (n as u64) * 4;
...
// COMMON-REWRITES-NEXT:         let {{__v[0-9]+}}: i32 = i * i;
