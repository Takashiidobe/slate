...
// REWRITES-X86_64-GNU-DAG:     core::arch::asm!("cmpl {4:e}, {3:e}\n\tsete {0:l}\n\tmovzbl {0:l}, {0:e}\n\tseta {1:l}\n\tmovzbl {1:l}, {1:e}\n\tsetl {2:l}\n\tmovzbl {2:l}, {2:e}", lateout(reg) {{__v[0-9]+}}, lateout(reg) {{__v[0-9]+}}, lateout(reg) {{__v[0-9]+}}, in(reg) {{arg[0-9]+}}, in(reg) {{arg[0-9]+}}, options(att_syntax));
// REWRITES-X86_64-GNU-DAG: }
// REWRITES-X86_64-GNU-DAG: let {{__v[0-9]+}}: i32;
// REWRITES-X86_64-GNU-DAG: unsafe {
// REWRITES-X86_64-GNU-DAG:     core::arch::asm!("cmp {1:e}, {2:e}\n\tsetz {0:l}\n\tmovzx {0:e}, {0:l}", lateout(reg) {{__v[0-9]+}}, in(reg) {{arg[0-9]+}}, in(reg) {{arg[0-9]+}});
// REWRITES-X86_64-GNU-DAG: }
// REWRITES-X86_64-GNU-DAG: let {{__v[0-9]+}}: u16;
// REWRITES-X86_64-GNU-DAG: let {{__v[0-9]+}}: u64;
// REWRITES-X86_64-GNU-DAG: unsafe {
// REWRITES-X86_64-GNU-DAG:     core::arch::asm!("testl {2:e}, {2:e}\n\tsets {0:l}\n\tmovzbw {0:l}, {0:x}\n\tsetne {1:l}\n\tmovzbq {1:l}, {1:r}", lateout(reg) {{__v[0-9]+}}, lateout(reg) {{__v[0-9]+}}, in(reg) {{arg[0-9]+}}, options(att_syntax));
// REWRITES-X86_64-GNU-DAG: }
