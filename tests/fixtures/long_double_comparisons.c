...
// LOWERING-NEXT:     let {{_v[0-9]+}}: LongDouble = tiny;
// LOWERING-NEXT:     let {{_v[0-9]+}}: LongDouble = zero;
...
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = unsafe { printf({{_v[0-9]+}} as *const i8, (tiny > zero) as i32, (zero < tiny) as i32, (one >= tiny) as i32, (tiny <= one) as i32) };
// REWRITES-NEXT: let {{_v[0-9]+}}: *mut i8 = b"%d %d %d %d\n\0".as_ptr() as *mut i8;
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = unsafe { printf({{_v[0-9]+}} as *const i8, (tiny < zero) as i32, (zero > tiny) as i32, (one <= tiny) as i32, (tiny >= one) as i32) };
// REWRITES-NEXT: __retval = 0;
