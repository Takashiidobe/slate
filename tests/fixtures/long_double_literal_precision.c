...
// LOWERING-NEXT: }
// LOWERING-EMPTY:
// LOWERING-NEXT: static mut global_pi: LongDouble = LongDouble([53, 194, 104, 33, 162, 218, 15, 201, 0, 64]);
// LOWERING-EMPTY:
...
// LOWERING-NEXT:     let {{__v[0-9]+}}: *mut i8 = b"local\0".as_ptr() as *mut i8;
// LOWERING-NEXT:     dump80({{__v[0-9]+}}, {{__v[0-9]+}});
// LOWERING-NEXT:     let {{__v[0-9]+}}: *mut i8 = b"global\0".as_ptr() as *mut i8;
// LOWERING-NEXT:     let {{__v[0-9]+}}: LongDouble = unsafe { global_pi };
// LOWERING-NEXT:     dump80({{__v[0-9]+}}, {{__v[0-9]+}});
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = 0;
...
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: static mut global_pi: LongDouble = LongDouble([53, 194, 104, 33, 162, 218, 15, 201, 0, 64]);
// REWRITES-EMPTY:
...
// REWRITES-NEXT:     dump80(c"local".as_ptr() as *mut i8, {{__v[0-9]+}});
