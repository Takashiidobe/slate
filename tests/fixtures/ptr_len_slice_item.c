...
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = sum_items({{__v[0-9]+}}, {{__v[0-9]+}});
...
// REWRITES-NEXT:             sum_items(unsafe {
// REWRITES-NEXT:                 std::slice::from_raw_parts({{__v[0-9]+}} as *const i32, (4 as i32) as usize)
// REWRITES-NEXT:             }),
// REWRITES-NEXT:         )
// REWRITES-NEXT:     };
...
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn sum_items({{arg[0-9]+}}: &[i32]) -> i32 {
