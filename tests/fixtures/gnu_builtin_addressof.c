...
// LOWERING-NEXT:     let {{_v[0-9]+}}: *mut i32 = address;
// LOWERING-NEXT:     let {{_v[0-9]+}}: bool = {{_v[0-9]+}} == std::ptr::addr_of_mut!(value);
...
// REWRITES-NEXT: let {{_v[0-9]+}}: bool = address == std::ptr::addr_of_mut!(value);
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = 37;
// REWRITES-NEXT: __retval = ({{_v[0-9]+}} as i32) + (((unsafe { *address }) == {{_v[0-9]+}}) as i32);
