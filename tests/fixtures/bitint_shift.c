...
// LOWERING-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// LOWERING-DAG:         {{__v[0-9]+}} << bitint::BUint::<129, 3, 24>::from_i128({{__v[0-9]+}} as i128);
// LOWERING-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// LOWERING-DAG:         {{__v[0-9]+}} >> bitint::BUint::<129, 3, 24>::from_u128({{__v[0-9]+}} as u128);
// LOWERING-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// LOWERING-DAG:         {{__v[0-9]+}} << bitint::BUint::<129, 3, 24>::from_i128({{__v[0-9]+}} as i128);
...
// LOWERING-DAG:         {{__v[0-9]+}} >> bitint::BInt::<256, 4, 32>::from_i128({{__v[0-9]+}} as i128);
...
// LOWERING-DAG:     let {{__v[0-9]+}}: bitint::BInt<256, 4, 32> = {{__v[0-9]+}} << {{__v[0-9]+}}.to_i128();
// LOWERING-DAG:     let {{__v[0-9]+}}: bitint::BInt<256, 4, 32> = {{__v[0-9]+}} >> {{__v[0-9]+}}.to_i128();
// LOWERING-DAG:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}}.to_i128() as i32;
// LOWERING-DAG:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}} + {{arg[0-9]+}};
// LOWERING-DAG:     return {{__v[0-9]+}};
...
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// REWRITES-DAG:         {{__v[0-9]+}} << bitint::BUint::<129, 3, 24>::from_i128((4 as i32) as i128);
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// REWRITES-DAG:         {{__v[0-9]+}} >> bitint::BUint::<129, 3, 24>::from_u128((3 as u32) as u128);
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// REWRITES-DAG:         {{__v[0-9]+}} << bitint::BUint::<129, 3, 24>::from_i128((5 as i64) as i128);
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// REWRITES-DAG:         {{__v[0-9]+}} >> bitint::BUint::<129, 3, 24>::from_i128(((2 as i16) as i32) as i128);
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// REWRITES-DAG:         {{__v[0-9]+}} << bitint::BUint::<129, 3, 24>::from_u128((6 as u64) as u128);
...
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> = bitint::BUint::<129, 3, 24>::from_decimal_str("1");
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// REWRITES-DAG:         {{__v[0-9]+}} << bitint::BUint::<129, 3, 24>::from_i128((128 as i32) as i128);
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BUint<129, 3, 24> =
// REWRITES-DAG:         {{__v[0-9]+}} >> bitint::BUint::<129, 3, 24>::from_i128((127 as i32) as i128);
...
// REWRITES-DAG:         {{__v[0-9]+}} >> bitint::BInt::<256, 4, 32>::from_i128((3 as i32) as i128);
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BInt<256, 4, 32> =
// REWRITES-DAG:         {{__v[0-9]+}} << bitint::BInt::<256, 4, 32>::from_i128((2 as i32) as i128);
...
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BInt<256, 4, 32> = {{__v[0-9]+}} << {{__v[0-9]+}}.to_i128();
// REWRITES-DAG:     let {{__v[0-9]+}}: bitint::BInt<256, 4, 32> = {{__v[0-9]+}} >> {{__v[0-9]+}}.to_i128();
// REWRITES-DAG:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}}.to_i128() as i32;
// REWRITES-DAG:     {{__v[0-9]+}} + {{arg[0-9]+}}
// REWRITES-DAG: }
