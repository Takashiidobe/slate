...
// LOWERING-NEXT:     let {{__v[0-9]+}}: u32 = 4000000000u32;
// LOWERING-NEXT:     let {{__v[0-9]+}}: u32 = 500000000;
// LOWERING-X86_64-GNU-NEXT:     let {{__v[0-9]+}}: *mut i8 = b"%u\n\0".as_ptr() as *mut i8;
// LOWERING-AARCH64-GNU-NEXT:     let {{__v[0-9]+}}: *mut u8 = b"%u\n\0".as_ptr() as *mut u8;
// LOWERING-NEXT:     let {{__v[0-9]+}}: u32 = uadd({{__v[0-9]+}}, {{__v[0-9]+}});
...
// LOWERING-NEXT:     let {{__v[0-9]+}}: u32 = {{arg[0-9]+}} + {{arg[0-9]+}};
...
// REWRITES-NEXT:     let {{__v[0-9]+}}: u32 = 4000000000u32;
// REWRITES-NEXT:     let {{__v[0-9]+}}: u32 = 500000000;
// REWRITES-NEXT:     unsafe { printf(c"%u\n".as_ptr(), uadd({{__v[0-9]+}}, {{__v[0-9]+}})) };
// REWRITES-NEXT:     unsafe { printf(c"%u\n".as_ptr(), {{__v[0-9]+}}) };
// REWRITES-NEXT:     unsafe { printf(c"%u\n".as_ptr(), {{__v[0-9]+}}) };
// REWRITES-NEXT:     std::process::exit(0 as i32);
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn uadd({{arg[0-9]+}}: u32, {{arg[0-9]+}}: u32) -> u32 {
