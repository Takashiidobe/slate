...
// LOWERING-NEXT:                     unsafe { fputs({{__v[0-9]+}} as *const core::ffi::c_char, {{__v[0-9]+}} as *mut libc::FILE) };
// LOWERING-NEXT:             }
// LOWERING-NEXT:         }
// LOWERING-NEXT:     }
// LOWERING-NEXT:     let {{__v[0-9]+}}: *mut libc::FILE = g;
...
// REWRITES-NEXT:     }
// REWRITES-NEXT:     unsafe { fputs(c"first\n".as_ptr(), f as *mut libc::FILE) };
// REWRITES-NEXT:     unsafe { fputs(c"second\n".as_ptr(), f as *mut libc::FILE) };
// REWRITES-NEXT:     unsafe { fputs(c"third".as_ptr(), f as *mut libc::FILE) };
// REWRITES-NEXT:     unsafe { fclose(f as *mut libc::FILE) };
// REWRITES-NEXT:     g = unsafe { fopen(c"slate_stdio_gets_loop_lines.tmp".as_ptr(), c"r".as_ptr()) };
