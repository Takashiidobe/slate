...
// REWRITES-DAG: let {{_v[0-9]+}}: *mut i8 = b"slate\0".as_ptr() as *mut i8;
// REWRITES-DAG: let {{_v[0-9]+}}: u64 = 6;
// REWRITES-DAG: let {{_v[0-9]+}}: u64 = 1;
// REWRITES-DAG: let {{_v[0-9]+}}: *mut Slice = make_slice(unsafe { &mut (*std::ptr::addr_of_mut!(_compoundliteral)) }, {{_v[0-9]+}}, {{_v[0-9]+}} - {{_v[0-9]+}});
...
// REWRITES-DAG: let {{_v[0-9]+}}: *mut i8 = b"translation\0".as_ptr() as *mut i8;
