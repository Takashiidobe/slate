...
// LOWERING-NEXT:             let {{_v[0-9]+}}: *mut cursor = c;
// LOWERING-NEXT:             let {{_v[0-9]+}}: bool = {{_v[0-9]+}} < unsafe { std::ptr::addr_of_mut!(*(*{{_v[0-9]+}}).buf.as_mut_ptr().add({{_v[0-9]+}} as usize)) };
...
// REWRITES-NEXT:         let {{_v[0-9]+}}: bool = {{_v[0-9]+}} < unsafe { std::ptr::addr_of_mut!(*(*{{_v[0-9]+}}).buf.as_mut_ptr().add({{_v[0-9]+}} as usize)) };
...
// REWRITES-NEXT: let {{_v[0-9]+}}: u8 = 1;
// REWRITES-NEXT: fill(unsafe { &mut (*std::ptr::addr_of_mut!(c)) }, {{_v[0-9]+}});
// REWRITES-NEXT: {
...
// REWRITES-NEXT: unsafe { printf({{_v[0-9]+}} as *const i8, total) };
