...
// REWRITES-NEXT:     let {{__v[0-9]+}}: *mut u8 = (unsafe { std::ptr::addr_of_mut!((*c).buf) }) as *mut u8;
// REWRITES-NEXT:     unsafe {
// REWRITES-NEXT:         (*c).p = {{__v[0-9]+}};
// REWRITES-NEXT:     }
// REWRITES-NEXT:     loop {
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut u8 = unsafe { (*c).p };
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut cursor = c;
// REWRITES-NEXT:         let {{__v[0-9]+}}: bool = {{__v[0-9]+}}
// REWRITES-NEXT:             < unsafe { std::ptr::addr_of_mut!(*(*{{__v[0-9]+}}).buf.as_mut_ptr().add((8 as u64) as usize)) };
