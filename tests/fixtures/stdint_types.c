...
// REWRITES-NEXT: struct FixedPair {
// REWRITES-NEXT:     left: i16,
// REWRITES-NEXT:     right: u32,
// REWRITES-NEXT:     count: u64,
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: static mut global_size: u64 = 7;
// REWRITES-EMPTY:
// REWRITES-NEXT: static mut global_u32: u32 = 4000000000;
// REWRITES-EMPTY:
...
// REWRITES-NEXT:     println!("{}", add_i32(10, 20));
// REWRITES-NEXT:     let _ = std::io::Write::flush(&mut std::io::stdout());
// REWRITES-NEXT:     let _ = std::io::Write::flush(&mut std::io::stdout());
// REWRITES-NEXT:     unsafe { printf(c"%lu\n".as_ptr(), widen_u32(5)) };
// REWRITES-NEXT:     unsafe { fflush(std::ptr::null_mut()) };
// REWRITES-NEXT:     println!("{}", use_fixed_types());
// REWRITES-NEXT:     let _ = std::io::Write::flush(&mut std::io::stdout());
// REWRITES-NEXT:     return std::process::ExitCode::SUCCESS;
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn add_i32({{arg[0-9]+}}: i32, {{arg[0-9]+}}: i16) -> i32 {
// REWRITES-NEXT:     {{arg[0-9]+}} + ({{arg[0-9]+}} as i32)
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: /// REWRITES-DAG: println!("{}
// REWRITES-NEXT: fn widen_u32({{arg[0-9]+}}: u32) -> u64 {
// REWRITES-NEXT:     ({{arg[0-9]+}} as u64) + unsafe { global_size }
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn use_fixed_types() -> i32 {
// REWRITES-NEXT:     let mut pair: FixedPair = FixedPair {
// REWRITES-NEXT:         left: 0,
// REWRITES-NEXT:         right: 0,
// REWRITES-NEXT:         count: 0,
// REWRITES-NEXT:     };
// REWRITES-NEXT:     let {{__v[0-9]+}}: i16 = 1200;
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = add_i32(100000, {{__v[0-9]+}});
// REWRITES-NEXT:     let {{__v[0-9]+}}: u64 = widen_u32(unsafe { global_u32 });
// REWRITES-NEXT:     pair.left = {{__v[0-9]+}};
// REWRITES-NEXT:     pair.right = unsafe { global_u32 };
// REWRITES-NEXT:     pair.count = (unsafe { global_size }) + 3;
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = (250 as u8) as i32;
// REWRITES-NEXT:     (((((-5 as i8) as i32) + {{__v[0-9]+}} + (pair.left as i32) + {{__v[0-9]+}} + ((65000 as u16) as i32)) as u64)
// REWRITES-NEXT:         + pair.count
