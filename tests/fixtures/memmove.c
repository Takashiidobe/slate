...
// LOWERING-DAG:                 break;
// LOWERING-DAG:             }
// LOWERING-DAG:             let {{__v[0-9]+}}: *mut i8 = b"%d \0".as_ptr() as *mut i8;
// LOWERING-DAG:             let {{__v[0-9]+}}: i32 = i;
...
// LOWERING-DAG:         }
// LOWERING-DAG:     }
// LOWERING-DAG:     let {{__v[0-9]+}}: *mut i8 = b"\n\0".as_ptr() as *mut i8;
...
// REWRITES-DAG:     let {{__v[0-9]+}}: *mut u8 = dyn_buf.as_mut_ptr() as *mut u8;
// REWRITES-DAG:     let {{__v[0-9]+}}: *mut u8 = unsafe { {{__v[0-9]+}}.add(1) };
// REWRITES-DAG:     unsafe { std::ptr::copy({{__v[0-9]+}} as *const u8, {{__v[0-9]+}} as *mut u8, ({{__v[0-9]+}} as u64) as usize) };
