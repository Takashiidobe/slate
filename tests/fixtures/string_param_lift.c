...
// REWRITES-NEXT: return unsafe { __slate_atoi({{arg[0-9]+}}.as_ptr() as *const i8) };
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn forward_num({{arg[0-9]+}}: &str) -> i32 {
// REWRITES-NEXT: return parse_num({{arg[0-9]+}});
// REWRITES-NEXT: }
// REWRITES-EMPTY:
...
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = text_len("hello");
...
// LOWERING-NEXT:     let {{_v[0-9]+}}: i32 = unsafe { atoi({{arg[0-9]+}} as *const i8) };
...
// LOWERING-NEXT:     let {{_v[0-9]+}}: u64 = (unsafe { strlen({{arg[0-9]+}} as *const i8) }) as u64;
