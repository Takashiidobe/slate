...
// REWRITES-EMPTY:
// REWRITES-EMPTY:
// REWRITES-NEXT: static mut alloc: Allocator = Allocator { realloc_fn: unsafe { std::mem::transmute::<*const (), Option<unsafe extern "C" fn(*mut core::ffi::c_void, u64) -> *mut core::ffi::c_void>>(realloc as *const ()) }, free_fn: unsafe { std::mem::transmute::<*const (), Option<unsafe extern "C" fn(*mut core::ffi::c_void)>>(free as *const ()) } };
// REWRITES-EMPTY:
...
// REWRITES-NEXT: let {{_v[0-9]+}}: *mut core::ffi::c_void = unsafe { unsafe { alloc.realloc_fn }.unwrap()({{_v[0-9]+}}, {{_v[0-9]+}}) };
// REWRITES-NEXT: unsafe { unsafe { alloc.free_fn }.unwrap()({{_v[0-9]+}}) };
