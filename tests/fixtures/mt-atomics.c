...
// LOWERING-NEXT:                 let {{__v[0-9]+}}: *mut __pthread_attr_t = std::ptr::null_mut();
...
// LOWERING-NEXT:                         std::ptr::addr_of_mut!(threads[({{__v[0-9]+}} as usize)]) as *mut u64,
// LOWERING-NEXT:                         {{__v[0-9]+}} as *const __pthread_attr_t,
...
// LOWERING-NEXT:                 let {{__v[0-9]+}}: i32 = i2;
// LOWERING-NEXT:                 let {{__v[0-9]+}}: i64 = {{__v[0-9]+}} as i64;
// LOWERING-NEXT:                 let {{__v[0-9]+}}: u64 = threads[({{__v[0-9]+}} as usize)];
// LOWERING-NEXT:                 let {{__v[0-9]+}}: *mut *mut core::ffi::c_void = std::ptr::null_mut();
// LOWERING-NEXT:                 let {{__v[0-9]+}}: i32 =
// LOWERING-NEXT:                     unsafe { pthread_join({{__v[0-9]+}} as u64, {{__v[0-9]+}} as *mut *mut core::ffi::c_void) };
// LOWERING-NEXT:             }
// LOWERING-NEXT:             let {{__v[0-9]+}}: i32 = i2;
// LOWERING-NEXT:             let {{__v[0-9]+}}: i32 = {{__v[0-9]+}} + 1;
// LOWERING-NEXT:             i2 = {{__v[0-9]+}};
...
// REWRITES-NEXT:         let {{__v[0-9]+}}: i64 = i as i64;
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut __pthread_attr_t = std::ptr::null_mut();
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut core::ffi::c_void = std::ptr::null_mut();
// REWRITES-NEXT:         unsafe {
// REWRITES-NEXT:             pthread_create(
// REWRITES-NEXT:                 std::ptr::addr_of_mut!(threads[({{__v[0-9]+}} as usize)]) as *mut u64,
// REWRITES-NEXT:                 {{__v[0-9]+}} as *const __pthread_attr_t,
...
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut *mut core::ffi::c_void = std::ptr::null_mut();
// REWRITES-NEXT:         unsafe {
// REWRITES-NEXT:             pthread_join(
// REWRITES-NEXT:                 threads[((i2 as i64) as usize)] as u64,
// REWRITES-NEXT:                 {{__v[0-9]+}} as *mut *mut core::ffi::c_void,
// REWRITES-NEXT:             )
// REWRITES-NEXT:         };
// REWRITES-NEXT:         i2 += 1;
// REWRITES-NEXT:     }
