...
// LOWERING-NEXT:     local_edge = {{_v[0-9]+}};
// LOWERING-NEXT:     let {{_v[0-9]+}}: *mut i8 = b"local\0".as_ptr() as *mut i8;
// LOWERING-NEXT:     let {{_v[0-9]+}}: LongDouble = local_edge;
...
// REWRITES-NEXT: dump80({{_v[0-9]+}}, local_edge);
// REWRITES-NEXT: let {{_v[0-9]+}}: *mut i8 = b"global\0".as_ptr() as *mut i8;
// REWRITES-NEXT: dump80({{_v[0-9]+}}, unsafe { global_edge });
// REWRITES-NEXT: __retval = 0;
