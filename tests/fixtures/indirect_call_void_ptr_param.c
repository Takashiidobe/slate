...
// LOWERING-NEXT:     }
// LOWERING-NEXT:     let {{_v[0-9]+}}: Option<unsafe extern "C" fn(*mut core::ffi::c_void, i32)> = handler;
// LOWERING-NEXT:     let {{_v[0-9]+}}: *mut Data = d;
...
// LOWERING-NEXT:             let {{_v[0-9]+}}: *mut i8 = c;
...
// REWRITES-NEXT: let mut d: *mut Data = {{arg[0-9]+}};
// REWRITES-NEXT: {
...
// REWRITES-NEXT: unsafe { handler.unwrap()(d as *mut core::ffi::c_void, {{_v[0-9]+}}) };
// REWRITES-NEXT: return;
// REWRITES-NEXT: }
// REWRITES-EMPTY:
...
// REWRITES-NEXT:                     let {{_v[0-9]+}}: i32 = unsafe { printf({{_v[0-9]+}} as *const i8, (unsafe { *c }) as i32) };
// REWRITES-NEXT:                     return;
// REWRITES-NEXT:         }
// REWRITES-NEXT: }
// REWRITES-NEXT: d = p as *mut Data;
// REWRITES-NEXT: let {{_v[0-9]+}}: *mut i8 = b"%d %d\n\0".as_ptr() as *mut i8;
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = unsafe { (*d).value };
// REWRITES-NEXT: let {{_v[0-9]+}}: i32 = unsafe { printf({{_v[0-9]+}} as *const i8, {{_v[0-9]+}}, extra) };
// REWRITES-NEXT: return;
// REWRITES-NEXT: }
// REWRITES-EMPTY:
