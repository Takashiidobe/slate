...
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = unsafe { printf({{__v[0-9]+}} as *const core::ffi::c_char, {{arg[0-9]+}}, {{arg[0-9]+}}) };
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = 0;
...
// REWRITES-NEXT:                 unsafe { std::slice::from_raw_parts({{__v[0-9]+}} as *const i32, 4usize) },
// REWRITES-NEXT:                 5,
// REWRITES-NEXT:                 4,
// REWRITES-NEXT:             ),
// REWRITES-NEXT:         )
...
// REWRITES-NEXT:     println!("this is another number: {} {}", {{arg[0-9]+}}, {{arg[0-9]+}});
// REWRITES-NEXT:     let _ = std::io::Write::flush(&mut std::io::stdout());
