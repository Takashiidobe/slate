...
// COMMON-LOWERING-NEXT:     forward_fill({{__v[0-9]+}}, {{__v[0-9]+}});
...
// COMMON-LOWERING-NEXT: }
// COMMON-LOWERING-EMPTY:
// COMMON-LOWERING-NEXT: fn forward_fill({{arg[0-9]+}}: *mut i32, {{arg[0-9]+}}: i32) {
// COMMON-LOWERING-NEXT:     fill_values({{arg[0-9]+}}, {{arg[0-9]+}});
// COMMON-LOWERING-NEXT:     return;
// COMMON-LOWERING-NEXT: }
// COMMON-LOWERING-EMPTY:
// COMMON-LOWERING-NEXT: fn fill_values({{arg[0-9]+}}: *mut i32, {{arg[0-9]+}}: i32) {
...
// COMMON-REWRITES-NEXT:     let {{__v[0-9]+}}: u64 = ({{__v[0-9]+}} as u64) * 4;
...
// COMMON-REWRITES-NEXT:     forward_fill(unsafe { std::slice::from_raw_parts_mut({{__v[0-9]+}} as *mut i32, {{__v[0-9]+}} as usize) });
...
// COMMON-REWRITES-NEXT: }
// COMMON-REWRITES-EMPTY:
// COMMON-REWRITES-NEXT: fn forward_fill({{arg[0-9]+}}: &mut [i32]) {
// COMMON-REWRITES-NEXT:     fill_values({{arg[0-9]+}});
// COMMON-REWRITES-NEXT:     return;
// COMMON-REWRITES-NEXT: }
// COMMON-REWRITES-EMPTY:
// COMMON-REWRITES-NEXT: fn fill_values({{arg[0-9]+}}: &mut [i32]) {
...
// COMMON-REWRITES-NEXT:         let {{__v[0-9]+}}: i32 = i * 3;
