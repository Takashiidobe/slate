...
// LOWERING-NEXT:     bump(std::ptr::addr_of_mut!(a));
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = 10;
// LOWERING-NEXT:     b = {{__v[0-9]+}};
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = peek(std::ptr::addr_of_mut!(b));
...
// LOWERING-NEXT:     }
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = use_and_free({{__v[0-9]+}});
...
// LOWERING-NEXT: }
// LOWERING-EMPTY:
// LOWERING-NEXT: fn peek({{arg[0-9]+}}: *mut i32) -> i32 {
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = unsafe { *{{arg[0-9]+}} };
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = 1;
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}} + {{__v[0-9]+}};
...
// REWRITES-NEXT:     b = 10;
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = peek(unsafe { &(*std::ptr::addr_of_mut!(b)) });
...
// REWRITES-NEXT:     unsafe {
// REWRITES-NEXT:         *({{arg[0-9]+}} as *mut i32) = (unsafe { *({{arg[0-9]+}} as *mut i32) }) + 1;
// REWRITES-NEXT:     }
// REWRITES-NEXT:     return;
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn peek({{arg[0-9]+}}: &i32) -> i32 {
// REWRITES-NEXT:     (unsafe { *({{arg[0-9]+}} as *const i32) }) + 1
// REWRITES-NEXT: }
// REWRITES-EMPTY:
...
// REWRITES-NEXT:     }
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = unsafe { *{{arg[0-9]+}} };
