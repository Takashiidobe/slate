...
// LOWERING-X86_64-GNU-NEXT:     let {{__v[0-9]+}}: *mut i8 = b"slate_stdio_fread_size_one.tmp\0".as_ptr() as *mut i8;
// LOWERING-X86_64-GNU-NEXT:     let {{__v[0-9]+}}: *mut i8 = b"slate_stdio_fread_size_one.tmp\0".as_ptr() as *mut i8;
...
// LOWERING-AARCH64-GNU-NEXT:     let {{__v[0-9]+}}: *mut u8 = b"slate_stdio_fread_size_one.tmp\0".as_ptr() as *mut u8;
// LOWERING-AARCH64-GNU-NEXT:     let {{__v[0-9]+}}: *mut u8 = b"slate_stdio_fread_size_one.tmp\0".as_ptr() as *mut u8;
...
// COMMON-REWRITES-NEXT:     unsafe { remove(c"slate_stdio_fread_size_one.tmp".as_ptr()) };
// COMMON-REWRITES-NEXT:     f = unsafe { fopen(c"slate_stdio_fread_size_one.tmp".as_ptr(), c"w".as_ptr()) };
...
// COMMON-REWRITES-NEXT:     }
// COMMON-REWRITES-NEXT:     unsafe { fputs(c"hello world".as_ptr(), f as *mut libc::FILE) };
// COMMON-REWRITES-NEXT:     unsafe { fclose(f as *mut libc::FILE) };
// COMMON-REWRITES-NEXT:     g = unsafe { fopen(c"slate_stdio_fread_size_one.tmp".as_ptr(), c"r".as_ptr()) };
