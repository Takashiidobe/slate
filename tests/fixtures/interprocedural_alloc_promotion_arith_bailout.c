...
// REWRITES-NEXT:     let {{_v[0-9]+}}: *mut i32 = Box::into_raw(alloc()).cast::<i32>();
// REWRITES-NEXT:     let {{_v[0-9]+}}: *mut i32 = unsafe { {{_v[0-9]+}}.add(1) };
// REWRITES-NEXT:     unsafe {
// REWRITES-NEXT:         *{{_v[0-9]+}} = 10;
// REWRITES-NEXT:     }
// REWRITES-NEXT:     unsafe { printf(c"%d\n".as_ptr(), unsafe { *{{_v[0-9]+}} }) };
// REWRITES-NEXT:     let {{_v[0-9]+}}: *mut i32 = unsafe { {{_v[0-9]+}}.offset((-1 as i32) as isize) };
