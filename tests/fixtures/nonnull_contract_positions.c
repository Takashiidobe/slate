...
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = combine(
...
// LOWERING-NEXT:     );
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = 2;
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = difference(
// LOWERING-NEXT:         std::ptr::addr_of_mut!(left),
// LOWERING-NEXT:         {{__v[0-9]+}},
// LOWERING-NEXT:         std::ptr::addr_of_mut!(right),
// LOWERING-NEXT:     );
...
// LOWERING-NEXT:     let {{__v[0-9]+}}: *mut i32 = optional;
...
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = (unsafe { *({{arg[0-9]+}} as *const i32) }) + {{arg[0-9]+}} * unsafe { *({{arg[0-9]+}} as *const i32) };
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = if optional != std::ptr::null_mut() {
// REWRITES-NEXT:         let {{__v[0-9]+}}: i32 = unsafe { *optional };
// REWRITES-NEXT:         {{__v[0-9]+}}
// REWRITES-NEXT:     } else {
