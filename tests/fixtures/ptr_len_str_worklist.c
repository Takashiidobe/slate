...
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = forward_text({{__v[0-9]+}}, {{__v[0-9]+}});
...
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = score_text({{arg[0-9]+}}, {{arg[0-9]+}});
...
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn forward_text({{arg[0-9]+}}: &str) -> i32 {
// REWRITES-NEXT:     score_text({{arg[0-9]+}})
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn score_text({{arg[0-9]+}}: &str) -> i32 {
// REWRITES-NEXT:     let mut bytes: *mut u8 = {{arg[0-9]+}}.as_ptr() as *mut u8;
// REWRITES-NEXT:     let mut len: i32 = {{arg[0-9]+}}.len() as i32;
// REWRITES-NEXT:     let mut score: i32 = 0;
// REWRITES-NEXT:     (unsafe { {{arg[0-9]+}}.len() as u64 }) as u64;
// REWRITES-NEXT:     for i in 0..len {
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut u8 = bytes;
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut u8 = unsafe { {{__v[0-9]+}}.offset((i as i64) as isize) };
// REWRITES-NEXT:         score += (unsafe { *{{__v[0-9]+}} }) as i32;
// REWRITES-NEXT:     }
// REWRITES-NEXT:     score
// REWRITES-NEXT: }
