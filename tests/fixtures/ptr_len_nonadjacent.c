...
// LOWERING-X86_64-GNU-NEXT:     let {{__v[0-9]+}}: *mut i8 = b"this is another number: %d\n\0".as_ptr() as *mut i8;
...
// LOWERING-AARCH64-GNU-NEXT:     let {{__v[0-9]+}}: *mut u8 = b"this is another number: %d\n\0".as_ptr() as *mut u8;
...
// COMMON-REWRITES-NEXT:                 unsafe { std::slice::from_raw_parts({{__v[0-9]+}} as *const i32, (4 as i32) as usize) },
...
// COMMON-REWRITES-NEXT: }
// COMMON-REWRITES-EMPTY:
// COMMON-REWRITES-NEXT: fn sum_items({{arg[0-9]+}}: &[i32], {{arg[0-9]+}}: i32) -> i32 {
// COMMON-REWRITES-NEXT:     let mut items: *mut i32 = {{arg[0-9]+}}.as_ptr() as *mut i32;
// COMMON-REWRITES-NEXT:     let mut length: i32 = {{arg[0-9]+}}.len() as i32;
// COMMON-REWRITES-NEXT:     let mut total: i32 = 0;
// COMMON-REWRITES-NEXT:     unsafe { printf(c"this is another number: %d\n".as_ptr(), {{arg[0-9]+}}) };
// COMMON-REWRITES-NEXT:     for i in 0..length {
