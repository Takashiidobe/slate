...
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = sum_items({{__v[0-9]+}}, {{__v[0-9]+}}, {{__v[0-9]+}});
...
// REWRITES-NEXT:                 unsafe { std::slice::from_raw_parts({{__v[0-9]+}} as *const i32, (4 as i32) as usize) },
// REWRITES-NEXT:                 5,
// REWRITES-NEXT:             ),
// REWRITES-NEXT:         )
// REWRITES-NEXT:     };
...
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn sum_items({{arg[0-9]+}}: &[i32], {{arg[0-9]+}}: i32) -> i32 {
// REWRITES-NEXT:     let mut items: *mut i32 = {{arg[0-9]+}}.as_ptr() as *mut i32;
// REWRITES-NEXT:     let mut length: i32 = {{arg[0-9]+}}.len() as i32;
// REWRITES-NEXT:     let mut total: i32 = 0;
// REWRITES-NEXT:     unsafe { printf(c"this is another number: %d\n".as_ptr(), {{arg[0-9]+}}) };
// REWRITES-NEXT:     for i in 0..length {
