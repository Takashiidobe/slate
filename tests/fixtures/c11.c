...
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}} + ((({{__v[0-9]+}} as u64) % 64 == 0) as i32);
// REWRITES-NEXT:     let {{__v[0-9]+}}: u64 = std::ptr::addr_of_mut!(*aligned_object) as u64;
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}} + (({{__v[0-9]+}} % 32 == 0) as i32);
...
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = 11 + {{__v[0-9]+}} + 33;
...
// REWRITES-NEXT:     }
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = c11_evaluation_step(2);
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = c11_evaluation_step(3);
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}} + {{__v[0-9]+}} + unsafe { c11_evaluation_total };
// REWRITES-NEXT:     ref_tmp0 = c11_make_temporary(43);
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = ref_tmp0.values[1];
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = 0;
// REWRITES-NEXT:     let {{__v[0-9]+}}: i32 = 0 + {{__v[0-9]+}} + 1 + 1 + 1 + 1;
// REWRITES-NEXT:     unsafe {
...
// REWRITES-NEXT:         concurrency_total += ({{__v[0-9]+}} == 0) as i32;
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut core::ffi::c_void = unsafe { tss_get(thread_key as u32) };
// REWRITES-NEXT:         let {{__v[0-9]+}}: *mut core::ffi::c_void =
// REWRITES-NEXT:             std::ptr::addr_of_mut!(thread_increment) as *mut core::ffi::c_void;
// REWRITES-NEXT:         concurrency_total += ({{__v[0-9]+}} == {{__v[0-9]+}}) as i32;
...
// REWRITES-NEXT:     unsafe {
// REWRITES-NEXT:         *{{__v[0-9]+}} = {{arg[0-9]+}} + 1;
// REWRITES-NEXT:     }
// REWRITES-NEXT:     let {{__v[0-9]+}}: *mut i32 = unsafe { {{__v[0-9]+}}.add(2) };
// REWRITES-NEXT:     unsafe {
// REWRITES-NEXT:         *{{__v[0-9]+}} = {{arg[0-9]+}} + 2;
// REWRITES-NEXT:     }
...
// REWRITES-NEXT:     let {{__v[0-9]+}}: *mut i32 = unsafe { __errno_location() };
// REWRITES-NEXT:     {{__v[0-9]+}} + (((unsafe { *{{__v[0-9]+}} }) == 34) as i32)
// REWRITES-NEXT: }
// REWRITES-EMPTY:
