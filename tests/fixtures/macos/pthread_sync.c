...
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: *mut core::ffi::c_void = std::ptr::null_mut();
// LOWERING-MACOS-DAG:     result = {{__v[0-9]+}};
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: *mut *mut _opaque_pthread_t = thread;
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: *mut _opaque_pthread_attr_t = std::ptr::null_mut();
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: i32 = unsafe {
// LOWERING-MACOS-DAG:         pthread_create(
// LOWERING-MACOS-DAG:             {{__v[0-9]+}} as *mut *mut _opaque_pthread_t,
// LOWERING-MACOS-DAG:             {{__v[0-9]+}} as *const _opaque_pthread_attr_t,
...
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: *mut *mut _opaque_pthread_t = thread;
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: *mut _opaque_pthread_t = unsafe { *{{__v[0-9]+}} };
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: i32 = unsafe {
// LOWERING-MACOS-DAG:             pthread_join(
// LOWERING-MACOS-DAG:                 {{__v[0-9]+}} as *mut _opaque_pthread_t,
...
// LOWERING-MACOS-DAG:             let {{__v[0-9]+}}: *mut libc::timespec = deadline;
// LOWERING-MACOS-DAG:             let {{__v[0-9]+}}: i32 = unsafe {
...
// LOWERING-MACOS-DAG:                     {{__v[0-9]+}} as *const libc::timespec,
// LOWERING-MACOS-DAG:                 )
// LOWERING-MACOS-DAG:             };
// LOWERING-MACOS-DAG:             result = {{__v[0-9]+}};
// LOWERING-MACOS-DAG:         }
...
// LOWERING-MACOS-DAG:         {{__v[0-9]+}}
// LOWERING-MACOS-DAG:     } else {
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: *mut u64 = key;
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: Option<unsafe extern "C-unwind" fn(*mut core::ffi::c_void)> = None;
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: i32 = unsafe { pthread_key_create({{__v[0-9]+}} as *mut u64, {{__v[0-9]+}}) };
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: bool = {{__v[0-9]+}} != 0;
// LOWERING-MACOS-DAG:         {{__v[0-9]+}}
// LOWERING-MACOS-DAG:     };
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: bool = if {{__v[0-9]+}} {
...
// LOWERING-MACOS-DAG:     } else {
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: *mut u64 = key;
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: u64 = unsafe { *{{__v[0-9]+}} };
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: *mut core::ffi::c_void = value;
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: i32 =
// LOWERING-MACOS-DAG:             unsafe { pthread_setspecific({{__v[0-9]+}} as u64, {{__v[0-9]+}} as *const core::ffi::c_void) };
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: bool = {{__v[0-9]+}} != 0;
// LOWERING-MACOS-DAG:         {{__v[0-9]+}}
// LOWERING-MACOS-DAG:     };
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}} as i32;
...
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: i32 = 0;
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: *mut i32 = unsafe { sem_open({{arg[0-9]+}} as *const core::ffi::c_char, {{__v[0-9]+}} as i32) };
// LOWERING-MACOS-DAG:     semaphore = {{__v[0-9]+}};
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: *mut i32 = semaphore;
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: u64 = 18446744073709551615u64;
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: *mut i32 = {{__v[0-9]+}} as *mut i32;
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: bool = {{__v[0-9]+}} == {{__v[0-9]+}};
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: bool = if {{__v[0-9]+}} {
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: bool = true;
// LOWERING-MACOS-DAG:         {{__v[0-9]+}}
// LOWERING-MACOS-DAG:     } else {
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: *mut i32 = semaphore;
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: i32 = unsafe { sem_post({{__v[0-9]+}} as *mut i32) };
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: bool = {{__v[0-9]+}} != 0;
// LOWERING-MACOS-DAG:         {{__v[0-9]+}}
// LOWERING-MACOS-DAG:     };
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: bool = if {{__v[0-9]+}} {
...
// LOWERING-MACOS-DAG:     } else {
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: *mut i32 = semaphore;
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: i32 = unsafe { sem_close({{__v[0-9]+}} as *mut i32) };
// LOWERING-MACOS-DAG:         let {{__v[0-9]+}}: bool = {{__v[0-9]+}} != 0;
// LOWERING-MACOS-DAG:         {{__v[0-9]+}}
// LOWERING-MACOS-DAG:     };
// LOWERING-MACOS-DAG:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}} as i32;
