...
// LOWERING-X86_64-GNU-NEXT:     let {{__v[0-9]+}}: *mut i8 = b"%c%c %d\n\0".as_ptr() as *mut i8;
// LOWERING-AARCH64-GNU-NEXT:     let {{__v[0-9]+}}: *mut u8 = b"%c%c %d\n\0".as_ptr() as *mut u8;
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = 97;
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = unsafe { toupper({{__v[0-9]+}} as i32) };
...
// LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = {{__v[0-9]+}}.wrapping_abs();
...
// REWRITES-NEXT:             c"%c%c %d\n".as_ptr(),
// REWRITES-NEXT:             unsafe { toupper(97 as i32) },
// REWRITES-NEXT:             unsafe { tolower(90 as i32) },
// REWRITES-NEXT:             (-42 as i32).wrapping_abs(),
// REWRITES-NEXT:         )
// REWRITES-NEXT:     };
