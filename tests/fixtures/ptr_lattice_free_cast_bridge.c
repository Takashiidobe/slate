...
// COMMON-LOWERING-NEXT:     just_free({{__v[0-9]+}});
// COMMON-LOWERING-NEXT:     let {{__v[0-9]+}}: i32 = 0;
// COMMON-LOWERING-NEXT:     std::process::exit({{__v[0-9]+}} as i32);
// COMMON-LOWERING-NEXT: }
// COMMON-LOWERING-EMPTY:
// COMMON-LOWERING-NEXT: fn just_free({{arg[0-9]+}}: *mut i32) {
...
// COMMON-REWRITES-NEXT:         *{{__v[0-9]+}} = 4;
// COMMON-REWRITES-NEXT:     }
// COMMON-REWRITES-NEXT:     unsafe { printf(c"%d\n".as_ptr(), unsafe { *{{__v[0-9]+}} }) };
// COMMON-REWRITES-NEXT:     just_free({{__v[0-9]+}});
// COMMON-REWRITES-NEXT:     std::process::exit(0 as i32);
// COMMON-REWRITES-NEXT: }
// COMMON-REWRITES-EMPTY:
// COMMON-REWRITES-NEXT: fn just_free({{arg[0-9]+}}: *mut i32) {
