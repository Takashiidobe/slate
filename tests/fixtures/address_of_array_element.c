...
// LOWERING-NEXT:     let {{_v[0-9]+}}: i32 = read_pointer(std::ptr::addr_of_mut!(values[({{_v[0-9]+}} as usize)]));
// LOWERING-NEXT:     return {{_v[0-9]+}};
// LOWERING-NEXT: }
// LOWERING-EMPTY:
...
// LOWERING-NEXT:     let {{_v[0-9]+}}: i64 = unsafe { std::ptr::addr_of_mut!(values[({{_v[0-9]+}} as usize)]).offset_from(std::ptr::addr_of_mut!(values[({{_v[0-9]+}} as usize)])) as i64 };
