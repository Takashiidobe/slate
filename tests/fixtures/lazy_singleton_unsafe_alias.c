...
// REWRITES-NEXT:     let {{__v[0-9]+}}: bool = !((unsafe { computed }) != 0);
...
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: /// Reads the payload outside the guarded init, so the singleton fixup must
// REWRITES-NEXT: /// not fire: the static isn't exclusively owned by get_value anymore.
// REWRITES-NEXT: fn peek_value() -> i32 {
// REWRITES-NEXT:     unsafe { cached_value }
// REWRITES-NEXT: }
// REWRITES-EMPTY:
// REWRITES-NEXT: fn main() {
// REWRITES-NEXT:     println!("{}", get_value());
// REWRITES-NEXT:     let _ = std::io::Write::flush(&mut std::io::stdout());
// REWRITES-NEXT:     println!("{}", peek_value());
// REWRITES-NEXT:     let _ = std::io::Write::flush(&mut std::io::stdout());
