...
// LOWERING-NEXT:     let _v2: *mut i8 = b"sinl\0".as_ptr() as *mut i8;
// LOWERING-NEXT:     let _v3: LongDouble = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
// LOWERING-NEXT:     let _v4: LongDouble = unsafe { __slate_sinl__rf80_f80(_v3) };
// LOWERING-NEXT:     dump80(_v2, _v4);
...
// REWRITES-NEXT: let _v1: LongDouble = LongDouble([0, 0, 0, 0, 0, 0, 0, 128, 255, 63]);
// REWRITES-NEXT: unsafe { std::ptr::write_volatile(std::ptr::addr_of_mut!(x), _v1) };
// REWRITES-NEXT: let _v2: *mut i8 = b"sinl\0".as_ptr() as *mut i8;
// REWRITES-NEXT: let _v3: LongDouble = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
// REWRITES-NEXT: let _v4: LongDouble = unsafe { __slate_sinl__rf80_f80(_v3) };
// REWRITES-NEXT: dump80(_v2, _v4);
// REWRITES-NEXT: __retval = 0;
