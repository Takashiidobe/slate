...
// LOWERING-NEXT:     let {{__v[0-9]+}}: *mut i8 = b"sinl\0".as_ptr() as *mut i8;
// LOWERING-NEXT:     let {{__v[0-9]+}}: LongDouble = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
...
// REWRITES-NEXT:     let {{__v[0-9]+}}: *mut i8 = c"sinl".as_ptr() as *mut i8;
// REWRITES-NEXT:     let {{__v[0-9]+}}: LongDouble = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
// REWRITES-NEXT:     dump80({{__v[0-9]+}}, unsafe { __slate_sinl__rf80_f80({{__v[0-9]+}}) });
