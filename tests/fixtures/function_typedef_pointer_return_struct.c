...
// LOWERING-NEXT:     let {{__v[0-9]+}}: Callback = Callback { handler: None };
...
// LOWERING-NEXT:     let {{__v[0-9]+}}: Option<unsafe extern "C-unwind" fn(*mut i32) -> *mut i32> = callback.handler;
// LOWERING-NEXT:     let {{__v[0-9]+}}: *mut i32 = unsafe { {{__v[0-9]+}}.unwrap()(std::ptr::addr_of_mut!(value)) };
...
// REWRITES-NEXT:     let {{__v[0-9]+}}: *mut i32 = unsafe { callback.handler.unwrap()(std::ptr::addr_of_mut!(value)) };
// REWRITES-NEXT:     unsafe { printf({{__v[0-9]+}} as *const core::ffi::c_char, unsafe { *{{__v[0-9]+}} }) };
