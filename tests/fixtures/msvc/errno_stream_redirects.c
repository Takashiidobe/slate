...
// REWRITES-MSVC-DAG:     }
// REWRITES-MSVC-DAG:     let {{__v[0-9]+}}: *mut i8 = c"hi\n".as_ptr() as *mut i8;
// REWRITES-MSVC-DAG:     let {{__v[0-9]+}}: u32 = 1;
// REWRITES-MSVC-DAG:     let {{__v[0-9]+}}: *mut _iobuf = unsafe { __acrt_iob_func({{__v[0-9]+}} as u32) };
// REWRITES-MSVC-DAG:     unsafe { fputs({{__v[0-9]+}} as *const core::ffi::c_char, {{__v[0-9]+}} as *mut _iobuf) };
// REWRITES-MSVC-DAG:     let {{__v[0-9]+}}: *mut i8 = c"bye\n".as_ptr() as *mut i8;
// REWRITES-MSVC-DAG:     let {{__v[0-9]+}}: u32 = 2;
// REWRITES-MSVC-DAG:     let {{__v[0-9]+}}: *mut _iobuf = unsafe { __acrt_iob_func({{__v[0-9]+}} as u32) };
// REWRITES-MSVC-DAG:     unsafe { fputs({{__v[0-9]+}} as *const core::ffi::c_char, {{__v[0-9]+}} as *mut _iobuf) };
// REWRITES-MSVC-DAG:     let {{__v[0-9]+}}: *mut i32 = unsafe { _errno() };
// REWRITES-MSVC-DAG:     let {{__v[0-9]+}}: i32 = unsafe { *{{__v[0-9]+}} };
