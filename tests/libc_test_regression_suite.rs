...
fn supported_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures.libc-test/regression/supported")
}

fn unsupported_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures.libc-test/regression/unsupported")
}

fn collect_cases(dir: &Path) -> Vec<(String, PathBuf)> {
    let selected = std::env::var("SLATE_LIBC_TEST_REGRESSION_FIXTURE").ok();
...
fn run_cases(group: &str, dir: &Path) -> Vec<(String, Result<(), String>)> {
    ...
    let work = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("target/libc-test-regression-suite")
        .join(group);
    ...
    let translated = support::parallel_map_with_jobs(&cases, jobs, |(name, path)| {
        ...
        let dso = path.with_file_name(format!("{name}_dso.so"));
        let mut config = support::RunConfig {
            timeout_seconds: Some(5),
            ..support::RunConfig::default()
        };
        if dso.is_file() {
            config.extra_files = vec![dso];
            config.extra_files_dir = PathBuf::new();
            config
                .env
                .insert("LD_LIBRARY_PATH".to_string(), ".".to_string());
        }
...
#[test]
fn libc_test_regression_supported_tests_match_c() {
    ...
    assert!(
        failures.is_empty(),
        "libc-test regression supported tests failed:\n{}",
        failures.join("\n\n")
    );
}
...
fn libc_test_regression_unsupported_tests_still_fail() {
    ...
    assert!(
        ...
        "libc-test regression test(s) now pass end-to-end -- promote them:\n{}",
        unexpected_passes
            .iter()
            .map(|name| format!(
                "  git mv tests/fixtures.libc-test/regression/unsupported/{name}.c tests/fixtures.libc-test/regression/supported/{name}.c"
            ))
            .collect::<Vec<_>>()
            .join("\n")
    );
