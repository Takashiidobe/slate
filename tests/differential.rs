...
fn ptr_len_pairs_use_slice_params_for_full_array_calls() {
    ...
    assert!(rust.contains("for __slate_item in items.iter()"));
    assert!(rust.contains("for __slate_item in items.iter_mut()"));
    assert!(rust.contains("let _v7: i32 = *__slate_item;"));
    assert!(rust.contains("*__slate_item = _v8;"));
