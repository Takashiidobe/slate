...
static void f() {
  ...
  C(EAI_SOCKTYPE)
  C(EAI_SYSTEM)
  C(EAI_OVERFLOW) { void (*p)(void) = endhostent; }
  {
...
static void g() {
  ...
  {
    int (*p)(const char *restrict, const char *restrict,
             const struct addrinfo *restrict, struct addrinfo **restrict) =
        getaddrinfo;
  }
  {
    int (*p)(const struct sockaddr *restrict, socklen_t, char *restrict,
             socklen_t, char *restrict, socklen_t, int) = getnameinfo;
  }
}
