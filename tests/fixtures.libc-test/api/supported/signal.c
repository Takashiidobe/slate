...
#define C(n)                                                                   \
  switch (n) { case n:; }
static void f() {
  T(sig_atomic_t) { void (*x)(int) = SIG_DFL; }
  {
    void (*x)(int) = SIG_ERR;
  }
  {
...
#endif
  {
    ...
    F(sigset_t, sa_mask)
    F(int, sa_flags) { void (**y)(int, siginfo_t *, void *) = &x.sa_sigaction; }
  }
  C(SIG_BLOCK)
  C(SIG_UNBLOCK)
  C(SIG_SETMASK)
  ...
  C(SI_TIMER)
  C(SI_ASYNCIO)
  C(SI_MESGQ) { int (*p)(pid_t, int) = kill; }
  {
  ...
  {
    int (*p)(int, const sigset_t *restrict, sigset_t *restrict) =
        pthread_sigmask;
  }
  {
    int (*p)(int, const struct sigaction *restrict,
             struct sigaction *restrict) = sigaction;
  }
  {
  ...
  {
    int (*p)(const sigset_t *restrict, siginfo_t *restrict,
             const struct timespec *restrict) = sigtimedwait;
  }
  {
