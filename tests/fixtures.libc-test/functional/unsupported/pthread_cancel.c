...
static void cleanup1(void *arg) { *(int *)arg = 1; }

static void cleanup2(void *arg) { *(int *)arg += 2; }

static void cleanup3(void *arg) { *(int *)arg += 3; }

static void cleanup4(void *arg) { *(int *)arg += 4; }
...
int main(void) {
  ...
  TESTR(r, pthread_create(&td, 0, start_async, &sem1),
  ...
  foo[0] = 0;
  TESTR(r, pthread_create(&td, 0, start_single, foo),
        "failed to create thread");
  ...
  TESTR(r, pthread_create(&td, 0, start_nested, foo),
        "failed to create thread");
  TESTR(r, pthread_cancel(td), "cancelling");
  TESTR(r, pthread_join(td, &res), "joining canceled thread");
  TESTC(res == PTHREAD_CANCELED, "canceled thread exit status");
