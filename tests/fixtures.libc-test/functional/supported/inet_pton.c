...
#define V6(src, ret, hex)                                                      \
  do {                                                                         \
    ...
    if (inet_ntop(AF_INET6, binaddr, txtaddr, sizeof txtaddr) != txtaddr)      \
      t_error("inet_ntop(AF_INET6, <" #hex                                     \
              ">, buf, size) did not return buf\n");                           \
    if (inet_pton(AF_INET6, txtaddr, binaddr) != 1)                            \
      t_error("inet_ntop(AF_INET6, <" #hex                                     \
              ">, buf, size) got %s, it is rejected by inet_pton\n",           \
              txtaddr);                                                        \
    tohex(hexaddr, binaddr, 16);                                               \
    if (strcmp(hexaddr, hex))                                                  \
      t_error("inet_ntop(AF_INET6, <" #hex                                     \
              ">, buf, size) got %s that is %s, want %s\n",                    \
              txtaddr, hexaddr, hex);                                          \
    if (strncmp(hex, "00000000000000000000ffff", 24) == 0 &&                   \
        !strchr(txtaddr, '.'))                                                 \
      t_error("inet_ntop(AF_INET6, <" #hex                                     \
              ">, buf, size) got %s, should be ipv4 mapped\n",                 \
              txtaddr);                                                        \
  } while (0);
