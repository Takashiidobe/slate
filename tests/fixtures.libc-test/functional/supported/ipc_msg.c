...
static void snd() {
  ...
  if (msgget(k, IPC_CREAT | IPC_EXCL | 0666) != -1 || errno != EEXIST)
    t_error(
        "msgget(IPC_CREAT|IPC_EXCL) should have failed with EEXIST, got %s\n",
        strerror(errno));
...
static void rcv() {
  ...
  errno = 0;
  if (msgrcv(qid, &msg, sizeof msg.data, msgtyp, MSG_NOERROR | IPC_NOWAIT) !=
          -1 ||
      errno != ENOMSG)
    t_error(
        "msgrcv should have failed when ther is no msg with ENOMSG, got %s\n",
        strerror(errno));
