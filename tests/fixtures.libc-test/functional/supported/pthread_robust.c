...
#define TEST(r, f, m)                                                          \
  (((r) = (f)) == 0 ||                                                         \
   (t_error("%s failed: (pshared==%d, pi==%d) %s (" m ")\n", #f, pshared, pi,  \
            strerror(r)),                                                      \
    0))
#define TESTX(r, f, x, m)                                                      \
  (((r) = (f)) == (x) ||                                                       \
   (t_error(                                                                   \
        "%s failed: (pshared==%d, pi==%d) got %d \"%s\" want %d \"%s\" (" m    \
        ")\n",                                                                 \
        #f, pshared, pi, r, strerror(r), x, strerror(x)),                      \
    0))
...
void f(int pshared, int pi) {
  ...
  TEST(r, pthread_mutexattr_setrobust(&mtx_a, PTHREAD_MUTEX_ROBUST),
       "setting robust attribute");
  if (pshared)
    TEST(r, pthread_mutexattr_setpshared(&mtx_a, PTHREAD_PROCESS_SHARED),
         "setting pshared attribute");
  if (pi)
    TEST(r, pthread_mutexattr_setprotocol(&mtx_a, PTHREAD_PRIO_INHERIT),
         "setting PI attribute");
  ...
  TESTX(r, pthread_mutex_lock(&mtx), ENOTRECOVERABLE,
  ...
  if (r && r != PTHREAD_BARRIER_SERIAL_THREAD)
    t_error(
        "pthread_barrier_wait failed: got %d \"%s\", wanted either 0 or %d\n",
