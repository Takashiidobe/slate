...
#define TESTR(f, m)                                                            \
  do {                                                                         \
    int r;                                                                     \
    if ((r = (f)))                                                             \
      t_error(#f " failed: %s (%s, " m ")\n", strerror(r), cdescr);            \
  } while (0)
#define TESTE(f, m)                                                            \
  do {                                                                         \
    if ((f) == -1)                                                             \
      t_error(#f " failed: %s (%s, " m ")\n", strerror(errno), cdescr);        \
  } while (0)
...
static void execute_sem_timedwait(void *arg) {
  ...
  ts.tv_sec += 1;
  TESTR(sem_timedwait(&sem_test, &ts),
...
static struct {
  ...
  void       *arg;
  const char *descr;
} scenarios[] = {{1, prepare_sem, execute_sem_wait, cleanup_sem, 0,
                  "blocking sem_wait"},
                 {1, prepare_sem, execute_sem_wait, cleanup_sem, (void *)1,
                  "non-blocking sem_wait"},
                 {1, prepare_sem, execute_sem_timedwait, cleanup_sem, 0,
                  "blocking sem_timedwait"},
                 {1, prepare_sem, execute_sem_timedwait, cleanup_sem, (void *)1,
                  "non-blocking sem_timedwait"},
                 {1, prepare_thread, execute_thread_join, cleanup_thread, 0,
                  "blocking pthread_join"},
                 {1, prepare_thread, execute_thread_join, cleanup_thread,
                  (void *)1, "non-blocking pthread_join"},
                 {0, prepare_dummy, execute_shm_open, cleanup_shm, &(int){0},
                  "shm_open"},
                 {0}},
  *cur_sc     = scenarios;
...
int main(void) {
  ...
  for (; cur_sc->prepare; cur_sc++) {
    ...
    seqno = 0;
    TESTR(pthread_create(&td, 0, run_execute, 0),
          "creating thread to be canceled");
