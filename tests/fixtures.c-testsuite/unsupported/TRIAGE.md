...
case_00213: GNU statement expression (`({ ... })`) used as a suppressed branch of a ternary, with a label/goto inside it that's reachable from outside — most of the program's expected output is missing from the Rust run; statement-expression + goto/dead-code-reachability interaction bug.
