...
// LOWERING-DAG:     let {{_v[0-9]+}}: bitint::BInt<16, 1, 2> = bitint::BInt::<16, 1, 2>::from_buint({{arg[0-9]+}});
// LOWERING-DAG:     let {{_v[0-9]+}}: i32 = {{_v[0-9]+}}.to_i128() as i32;
...
// LOWERING-DAG:     let {{_v[0-9]+}}: bitint::BUint<16, 1, 2> = *a;
// LOWERING-DAG:     let {{_v[0-9]+}}: i32 = neg({{_v[0-9]+}});
// LOWERING-DAG:     let {{_v[0-9]+}}: bool = {{_v[0-9]+}} != 0;
// LOWERING-DAG:     let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = if {{_v[0-9]+}} {
// LOWERING-DAG:         let {{_v[0-9]+}}: bitint::BUint<16, 1, 2> = *a;
// LOWERING-DAG:         let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = bitint::BUint::<17, 1, 4>::from_buint({{_v[0-9]+}});
// LOWERING-DAG:         {{_v[0-9]+}}
// LOWERING-DAG:     } else {
// LOWERING-DAG:         let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = bitint::BUint::<17, 1, 4>::from_decimal_str("65535");
// LOWERING-DAG:         {{_v[0-9]+}}
// LOWERING-DAG:     };
// LOWERING-DAG:     return {{_v[0-9]+}};
// LOWERING-DAG: }
...
// LOWERING-DAG:     *a = {{arg[0-9]+}};
// LOWERING-DAG:     let {{_v[0-9]+}}: bitint::BUint<16, 1, 2> = *a;
// LOWERING-DAG:     let {{_v[0-9]+}}: bitint::BInt<16, 1, 2> = bitint::BInt::<16, 1, 2>::from_buint({{_v[0-9]+}});
// LOWERING-DAG:     let {{_v[0-9]+}}: i32 = {{_v[0-9]+}}.to_i128() as i32;
// LOWERING-DAG:     let {{_v[0-9]+}}: i32 = 0;
// LOWERING-DAG:     let {{_v[0-9]+}}: bool = {{_v[0-9]+}} < {{_v[0-9]+}};
// LOWERING-DAG:     let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = if {{_v[0-9]+}} {
...
// LOWERING-DAG:         loop {
// LOWERING-DAG:             let {{_v[0-9]+}}: u32 = i;
// LOWERING-DAG:             let {{_v[0-9]+}}: u64 = {{_v[0-9]+}} as u64;
// LOWERING-DAG:             let {{_v[0-9]+}}: u64 = 10;
...
// LOWERING-DAG:                 let {{_v[0-9]+}}: u32 = i;
// LOWERING-DAG:                 let {{_v[0-9]+}}: u64 = {{_v[0-9]+}} as u64;
// LOWERING-DAG:                 let {{_v[0-9]+}}: bitint::BUint<16, 1, 2> = unsafe { (*main_v)[({{_v[0-9]+}} as usize)] };
// LOWERING-DAG:                 let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = f({{_v[0-9]+}});
// LOWERING-DAG:                 let {{_v[0-9]+}}: u32 = i;
// LOWERING-DAG:                 let {{_v[0-9]+}}: u64 = {{_v[0-9]+}} as u64;
// LOWERING-DAG:                 let {{_v[0-9]+}}: bitint::BUint<16, 1, 2> = unsafe { (*main_v)[({{_v[0-9]+}} as usize)] };
// LOWERING-DAG:                 let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = fref({{_v[0-9]+}});
// LOWERING-DAG:                 let {{_v[0-9]+}}: bool = {{_v[0-9]+}} != {{_v[0-9]+}};
// LOWERING-DAG:                 if {{_v[0-9]+}} {
...
// LOWERING-DAG:             }
// LOWERING-DAG:             let {{_v[0-9]+}}: u32 = i;
// LOWERING-DAG:             let {{_v[0-9]+}}: u32 = {{_v[0-9]+}} + 1;
// LOWERING-DAG:             i = {{_v[0-9]+}};
// LOWERING-DAG:         }
// LOWERING-DAG:     }
// LOWERING-DAG:     let {{_v[0-9]+}}: i32 = 0;
...
// REWRITES-DAG:     let {{_v[0-9]+}}: bitint::BInt<16, 1, 2> = bitint::BInt::<16, 1, 2>::from_buint({{arg[0-9]+}});
...
// REWRITES-DAG:     let {{_v[0-9]+}}: i32 = neg(*a);
// REWRITES-DAG:     let {{_v[0-9]+}}: bool = {{_v[0-9]+}} != 0;
// REWRITES-DAG:     let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = if {{_v[0-9]+}} {
// REWRITES-DAG:         let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = bitint::BUint::<17, 1, 4>::from_buint(*a);
// REWRITES-DAG:         {{_v[0-9]+}}
// REWRITES-DAG:     } else {
// REWRITES-DAG:         let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = bitint::BUint::<17, 1, 4>::from_decimal_str("65535");
// REWRITES-DAG:         {{_v[0-9]+}}
// REWRITES-DAG:     };
...
// REWRITES-DAG:         aligned::Aligned(bitint::BUint::<16, 1, 2>::ZERO);
// REWRITES-DAG:     *a = {{arg[0-9]+}};
// REWRITES-DAG:     let {{_v[0-9]+}}: bitint::BInt<16, 1, 2> = bitint::BInt::<16, 1, 2>::from_buint(*a);
// REWRITES-DAG:     let {{_v[0-9]+}}: i32 = {{_v[0-9]+}}.to_i128() as i32;
// REWRITES-DAG:     let {{_v[0-9]+}}: bool = {{_v[0-9]+}} < 0;
// REWRITES-DAG:     let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = if {{_v[0-9]+}} {
// REWRITES-DAG:         let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = bitint::BUint::<17, 1, 4>::from_buint(*a);
...
// REWRITES-DAG:     i = 0;
// REWRITES-DAG:     loop {
// REWRITES-DAG:         let {{_v[0-9]+}}: u64 = 2;
// REWRITES-DAG:         if !((i as u64) < 10 / {{_v[0-9]+}}) {
// REWRITES-DAG:             break;
// REWRITES-DAG:         }
// REWRITES-DAG:         let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = f(unsafe { (*main_v)[((i as u64) as usize)] });
// REWRITES-DAG:         let {{_v[0-9]+}}: bitint::BUint<17, 1, 4> = fref(unsafe { (*main_v)[((i as u64) as usize)] });
