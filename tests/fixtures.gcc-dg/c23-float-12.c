...
// LOWERING-AARCH64-GNU-NEXT:             let {{__v[0-9]+}}: i32 = 113;
...
// LOWERING-AARCH64-GNU-NEXT:         let {{__v[0-9]+}}: f128 = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
// LOWERING-AARCH64-GNU-NEXT:         let {{__v[0-9]+}}: f128 = 1.92592994438723585305597794258492732E-34f128;
// LOWERING-NEXT:         let {{__v[0-9]+}}: bool = {{__v[0-9]+}} != {{__v[0-9]+}};
// LOWERING-NEXT:         if {{__v[0-9]+}} {
...
// REWRITES-NEXT:     unsafe { std::ptr::write_volatile(std::ptr::addr_of_mut!(x), {{__v[0-9]+}}) };
// REWRITES-X86_64-GNU-NEXT:     for i in 0..((64 as i32) - 1) {
// REWRITES-X86_64-GNU-NEXT:         let {{__v[0-9]+}}: LongDouble = __slate_f80_from_i32(2 as i32);
// REWRITES-X86_64-GNU-NEXT:         let {{__v[0-9]+}}: LongDouble = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
// REWRITES-AARCH64-GNU-NEXT:     for i in 0..(113 - 1) {
// REWRITES-AARCH64-GNU-NEXT:         let {{__v[0-9]+}}: f128 = (2 as i32) as f128;
// REWRITES-AARCH64-GNU-NEXT:         let {{__v[0-9]+}}: f128 = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
// REWRITES-NEXT:         unsafe { std::ptr::write_volatile(std::ptr::addr_of_mut!(x), {{__v[0-9]+}} / {{__v[0-9]+}}) };
// REWRITES-NEXT:     }
// REWRITES-X86_64-GNU-NEXT:     let {{__v[0-9]+}}: LongDouble = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
// REWRITES-X86_64-GNU-NEXT:     let {{__v[0-9]+}}: bool = {{__v[0-9]+}} != LongDouble([0, 0, 0, 0, 0, 0, 0, 128, 192, 63]);
// REWRITES-AARCH64-GNU-NEXT:     let {{__v[0-9]+}}: f128 = unsafe { std::ptr::read_volatile(std::ptr::addr_of!(x)) };
// REWRITES-AARCH64-GNU-NEXT:     let {{__v[0-9]+}}: f128 = 1.92592994438723585305597794258492732E-34f128;
